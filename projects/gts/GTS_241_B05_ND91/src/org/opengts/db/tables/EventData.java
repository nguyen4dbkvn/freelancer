    ...
    implements EventDataProvider, GeoPointProvider
{
    ...
    /* field definition */
    //tanvm extra fieds
    public static final String FLD_statusLastingTime	= "statusLastingTime";		//Add status lasting time
   //hienntd extra fieds
    public static final String FLD_Cua	= "Cua";//Add Cua
    public static final String FLD_DieuHoa	= "DieuHoa";//Add Dieu Hoa
    ...
    public static final String FLD_Speed30Sec             = "Speed30s";  //ThanhNgC add// fix age (seconds)
    ...
    private static final DBField StandardFieldInfo[] = {
        ...
        // Standard fields
        new DBField(FLD_statusLastingTime	, Long.TYPE	, DBField.TYPE_UINT32	, "Status Lasting Time"		, ""),	//Add status lasting time - VietH add
        new DBField(FLD_Cua	, Integer.TYPE	, DBField.TYPE_UINT32	, "Cua"		, ""),	//Add Cua
        new DBField(FLD_DieuHoa	, Integer.TYPE	, DBField.TYPE_UINT32	, "DieuHoa"		, ""),	//Add Dieu Hoa
        new DBField(FLD_Speed30Sec      , String.class  , DBField.TYPE_STRING(200)  , "Speed30s"                   , "utf8=true"),//ThanhNGc add
  ...
  //public static final String FLD_appliedPressure      = "appliedPressure";        // kPa
    ...
    **/
    ...
    //Add status lasting time - tanvm add - BEGIN
	/**
    *** Gets the status lasting time of this event
    *** @return The status lasting time of this event
    **/
    public long getStatusLastingTime()
    {
        return this.getFieldValue(FLD_statusLastingTime, 0);
    }

    /**
     *** Sets the status lasting time of this event in Unix/Epoch time
     *** @param v The status lasting time of this event
     **/
     public void setStatusLastingTime(long v)
     {
         this.setFieldValue(FLD_statusLastingTime, v);
     }
     //Add status lasting time - tanvm add - END
     
    // ------------------------------------------------------------------------
     //Add Cua - hienntd add - BEGIN
 	/**
     *** Gets the status lasting time of this event
     *** @return The status lasting time of this event
     **/
     public int getCua()
     {
         return this.getFieldValue(FLD_Cua, 0);
     }
      ...
      *** @param v The status lasting time of this event
      **/
      public void setCua(int v)
      {
          this.setFieldValue(FLD_Cua, v);
      }
      //Add Cua - hienntd add - END
       ...
       *** @return The status lasting time of this event
       **/
      public void setDieuHoa(int v)
      {
          this.setFieldValue(FLD_DieuHoa, v);
      }
       public int getDieuHoa()
       {
           return this.getFieldValue(FLD_DieuHoa, 0);
       }
       
       
     //Add setSpeed30s - Thanhngc add - BEGIN
       public void setSpeed30s(String b)
       {
           this.setFieldValue(FLD_Speed30Sec, b);
       }
       public String getSpeed30s()
       {
    	   try {
    		   return  this.getFieldValue(FLD_Speed30Sec, "");
		} catch (Exception e) {
			return "";
		}	    	            
       }
     //Add setSpeed30s - Thanhngc add - END
    ...
    **/
        ...
        throws SlowOperationException
    {
        ...
        try {
            ...
            rg = rgp.getReverseGeocode(addrGP, privLabel.getLocaleString()); // get the reverse-geocode
        } catch (Throwable th) {
            // ignore
        } finally {
    ...
    **/
        ...
        boolean isFleet, BasicPrivateLabel bpl)
    {
        ...
        if (isFleet) {
            ...
            int fleetIconNdx = EventData._getPushpinIconIndex("fleet", iconKeys, -1/*ICON_PUSHPIN_BLUE*/);
            if (fleetIconNdx >= 0) {
                if (debugPP) PushpinDebug("\"fleet\" name", iconKeys, fleetIconNdx);
                return fleetIconNdx;
            }
        ...
        if (allIconNdx >= 0) { // '0' is a valid index
            if (debugPP) PushpinDebug("\"all\" name", iconKeys, allIconNdx);
            return allIconNdx;
        }
        ...
        if (!isFleet && this.getIsLastEvent()) {
            ...
            if (lastIconNdx >= 0) { // '0' is a valid index
                if (debugPP) PushpinDebug("\"last\" name", iconKeys, lastIconNdx);
                return lastIconNdx;
            }
        ...
        if (dftIconNdx >= 0) {
            if (debugPP) PushpinDebug("default name", iconKeys, dftIconNdx);
            return dftIconNdx;
        }
    ...
    **/
    ...
    public  static final String KEY_DRIVER[]        = new String[] { "driver"         , "driverDesc"     };  // "Joe Smith"
    ...
    public String getFieldValueString(String key, String arg, BasicPrivateLabel bpl)
    {
        ...
        if (EventData._keyMatch(key,EventData.KEY_FUEL_LEVEL2)) {
            long pct = 0; // Math.round(this.getFuelLevel2() * 100.0);
            return pct + "%";
        } else
        ...
        if (EventData._keyMatch(key,EventData.KEY_GEOZONENAME)) {
            return this.getGeozoneDisplayName();
        }
        ...
        if (EventData._keyMatch(key,EventData.KEY_DRIVER)) {
            ...
            if (!StringTools.isBlank(driverID)) {
                try {
                    Driver driver = Driver.getDriver(this.getAccount(), driverID);
                    if (driver != null) {
                        // return DriverID description from Driver table
                        return driver.getDescription();
                    } else {
                        // Driver records does not exist, just return driverID
                        return driverID;
                    }
                } catch (DBException dbe) {
                    Print.logError("Unable to read Driver record: " + dbe);
                    return driverID;
                }
            }
            // next try EntityID
            int    entityType = this.getEntityType();
            String entityID   = (entityType == EntityManager.EntityType.DRIVER.getIntValue())? this.getEntityID() : null;
            if (!StringTools.isBlank(entityID)) {
                String desc = Device.getEntityDescription(this.getAccountID(), entityID, entityType);
        ...
        if (EventData._keyMatch(key,EventData.KEY_INPUT_MASK)) {
            int input = (int)this.getInputMask(); // bit mask
        ...
        if (EventData._keyMatch(key,EventData.KEY_OUTPUT_MASK)) {
            int output = (int)this.getOutputMask(); // bit mask
        ...
        if (EventData._keyMatch(key,EventData.KEY_TEMPERATURE)) {
            ...
            double tempC = this.getThermoAverage(argNdx);
