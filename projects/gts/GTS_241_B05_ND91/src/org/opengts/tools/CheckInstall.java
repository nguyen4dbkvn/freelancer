    ...
    public static void main(String argv[])
    {
        ...
        if (defaultProps != null) {
            ...
            if (smsType.equals(SMSOutboundGateway.GW_httpURL)) {
                sms.append(defaultProps.getString(SMSOutboundGateway.PROP_httpURL_url,""));
                smsIsFunctional = 1;
            } else
            ...
            if (!StringTools.isBlank(sendTestSMSTo) && (smsIsFunctional == 1)) {
                println(PFX+"Attempting to send test SMS message to '"+sendTestSMSTo+"' ...");
                try {
                    Account sysadmin = Account.getAccount("sysadmin");
                    SMSOutboundGateway gw = SMSOutboundGateway.GetSMSGateway(smsType);
                    if (sysadmin == null) {
                        println(PFX+"ERROR: Unable to send SMS (no 'sysadmin' account)");
                        addError("Unable to send a test SMS message.",
                                 "No 'sysadmin' account found.",
                                 "Please fix displayed errors and re-run CheckInstall.");
                    } else
                    if (gw == null) {
