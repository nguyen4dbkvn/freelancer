<?
	...
	if(@$_GET['cmd'] == 'latlng')
	{
		...
		$url = 'https://maps.googleapis.com/maps/api/geocode/json?latlng='.$search.'&oe=utf-8&key='.$gsValues['MAP_GOOGLE_KEY'];
	...
	if(@$_GET['cmd'] == 'address')
	{
		...
		$url = 'https://maps.googleapis.com/maps/api/geocode/json?address='.$search.'&oe=utf-8&key='.$gsValues['MAP_GOOGLE_KEY'];
		$data = @file_get_contents($url);
		$jsondata = json_decode($data,true);
