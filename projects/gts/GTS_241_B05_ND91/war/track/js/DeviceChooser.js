...
function deviceShowChooserList(elemNameID, elemNameDesc, list)
{
    ...
    if (PREDEFINED_CHOOSER_HTML) {
        ...
        if (DeviceChooserEnableSearch) {
            html += "<form id='"+ID_SEARCH_FORM+"' name='"+ID_SEARCH_FORM+"' method='get' action=\"javascript:true;\" target='_top' style='padding-left:5px;background-color:#B5DEF5;color:black;'>";
...
function deviceGetTableHTML(list, searchVal)
{
    ...
    if (searchTableHeaderHtml == null) {
        var h = "";
        // begin table HTML
        h += "<table id='"+ID_DEVSELECT_TABLE+"' class='"+CLASS_TABLE_COLUMN_SORTABLE+"' cellspacing='0' cellpadding='0' >\n";
        // table header
        h += "<thead>\n";
