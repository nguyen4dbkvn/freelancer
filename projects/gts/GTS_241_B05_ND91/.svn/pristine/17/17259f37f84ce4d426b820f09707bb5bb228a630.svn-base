    ...
    implements Constants
{
    ...
    public String load_Trang(String accountID,String allow)
    {
    	String TrangID="";
    	String chuoi="<table>";
    	DBCamera objcmr = new  DBCamera();
  		//ResultSet rs = objcmr.GetCamera(ngay, device, page, pagesize);
  		ResultSet rs = objcmr.selectTblTrang();
  		try{
  		while(rs.next())
  		{
  			TrangID=rs.getString(1);
  			
  			ResultSet rs1 = objcmr.selectTblQuyen(accountID,TrangID);
  			while(rs1.next())
  					{String s=rs1.getString(1);
  						if(s.equals("1"))
  						chuoi =chuoi
  						+"<tr><td align='left'>"+ rs.getString(2)+"</td>"
  						+"<td align='left'><input name='checkTrang' type='checkbox' checked='checked' value='"+rs.getString(1)+"' "+allow+" /></td></tr>";
  						else
  							chuoi =chuoi
  	  						+"<tr><td align='left'>"+rs.getString(2)+"</td>"
  	  						+"<td align='left'><input name='checkTrang' type='checkbox'  value='"+rs.getString(1)+"'  "+allow+"/></td></tr>";
        ...
        throws IOException
    {
        ...
        if (newAccount) {
            ...
            if (createAccountOK) {
                try {
                    ...
                    Account account  = Account.createNewAccount(currAcct, newAccountID, newPasswd,1,1,"GMT+07:00"); // saved Hienntd function. default account will be created with MPH speedunit, Miles distanceUnit, GMT timezone. new function will create KPH,Km, GMT+07:00
                    accountList      = Account.getAuthorizedAccounts(currAcct);
                    selAccount       = account;
    	...
    	out.print("hehe");
     // if (submitChange.equals("Change"));
    	//out.print("<script>alert('minh')</script>");
              ...
              //String menuURL    = EncodeMakeURL(reqState,RequestProperties.TRACK_BASE_URI(),PAGE_MENU_TOP);
                ...
                if (_listAccounts) {
                    ...
                    if (_allowView) { 
                        ...
                        out.write("<input type='submit' class='button' name='"+PARM_SUBMIT_VIEW+"' value='"+i18n.getString("SysAdminAccounts.view","View")+"'>");
                        out.write("</td>\n"); 
                    }
                    if (_allowEdit) { 
                        out.write("<td style='padding-left:5px;'>");
                        out.write("<input type='submit' class='button' name='"+PARM_SUBMIT_EDIT+"' value='"+i18n.getString("SysAdminAccounts.edit","Edit")+"'>");
                        out.write("</td>\n"); 
                    }
                    if (_allowLogin) {
                        out.write("<td style='padding-left:5px;'>");
                        out.write("<input type='submit' class='button' name='"+PARM_SUBMIT_LOGIN+"' value='"+i18n.getString("SysAdminAccounts.login","Login")+"' "+Onclick_ConfirmLogin(locale)+">");
                        out.write("</td>\n"); 
                    }
                    out.write("<td style='width:100%; text-align:left; padding-left:5px;'>");
                    if (_allowDelete) {
                        out.write("<input type='submit' class='button' name='"+PARM_SUBMIT_DEL+"' value='"+i18n.getString("SysAdminAccounts.delete","Delete")+"' "+Onclick_ConfirmDelete(locale)+">");
                    } else {
                        out.write("&nbsp;"); 
                    }
                    out.write("</td>\n"); 
                    ...
                    out.write(" <input type='submit' class='button' name='"+PARM_SUBMIT_NEW+"' value='"+i18n.getString("SysAdminAccounts.new","New")+"' style='margin-top:5px; margin-left:10px;'>\n");
                    out.write("</form>\n");
                    out.write("</div>\n");
                    out.write("<hr>\n");
                    }
                    ...
                    out.println(FormRow_TextField(PARM_ACCT_CREATED      , false        , i18n.getString("SysAdminAccounts.creationDate","Creation Date")+":"           , createStr, 40, 24));
                    out.println(FormRow_TextField(PARM_ACCT_LAST_LOGIN   , false        , i18n.getString("SysAdminAccounts.lastLoginDate","Last Login Date")+":"         , lastLoginStr, 40, 24));
                    ...
                    out.println(FormRow_ComboBox (PARM_ACCT_ACTIVE       , _editAccount , i18n.getString("SysAdminAccounts.isActive","Is Active")+":"                   , acctActive, ComboMap.getYesNoMap(locale), "", 40));
                    out.println(FormRow_TextField(PARM_ACCT_PASSWORD     , _editAccount , i18n.getString("SysAdminAccounts.password","Password")+":"                    , decodedPass, 40, 20));
                    ...
                    out.println(FormRow_TextField(PARM_ACCT_CONTACT_PHONE, _editAccount , i18n.getString("SysAdminAccounts.contactPhone","Contact Phone:")              , contactPhone, 40, 20));
                    out.println(FormRow_TextField(PARM_ACCT_CONTACT_EMAIL, _editAccount , i18n.getString("SysAdminAccounts.contactEmail","Contact Email:")              , contactEmail, 40, 100));
                    out.println(FormRow_TextField(PARM_ACCT_MAXDEV       , _editAccount , i18n.getString("SysAdminAccounts.maxDevices" ,"Maximum Devices")  +":"        , String.valueOf(acctMaxDev),  40, 7, i18n.getString("SysAdminAccounts.enter0ForUnlimited","(Enter '0' for unlimited)")));

                    /* ping count */
                    if (showMaxPingCount) {
                    out.print("<tr>");
                    out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap>"+i18n.getString("SysAdminAccounts.maxCommandCount","Max Commands/Locates")+":<hr></td>");
                    out.print("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_DATA+"'>");
                    out.print(Form_TextField(PARM_ACCT_MAX_PING  , editSysAdmin, String.valueOf(maxPingCnt)  , 40, 7));
                    out.print(" &nbsp;&nbsp;" + i18n.getString("SysAdminAccounts.totalCommandCount","Total") + ": ");
                    out.print(Form_TextField(PARM_ACCT_TOTAL_PING, false       , String.valueOf(totalPingCnt), 40, 7));
                    if (editSysAdmin) {
                    ...
                    if (showAccountManager && isSysAdmin) {
                        ...
                        out.println(FormRow_ComboBox (PARM_ACCT_IS_MANAGER, editSysAdmin , i18n.getString("SysAdminAccounts.isManager","Is Account Manager")+":"        , acctManager, ComboMap.getYesNoMap(locale), "", 40));
                        out.println(FormRow_TextField(PARM_ACCT_MANAGER_ID, editSysAdmin , i18n.getString("SysAdminAccounts.managerID","Account Manager ID")+":"        , acctManagerID, 40, 32));
                    }
                    ...
                    if (showDataPushURL && isSysAdmin) {
                        ...
                        out.println(FormRow_TextField(PARM_ACCT_DATA_PUSH_URL, editSysAdmin, i18n.getString("SysAdminAccounts.dataPushURL","Data Push URL")+":"         , dpURL, 40, 200));
                    }

                    /* privateLabel, geocoderMode, isBorderCrossing */
                    out.println(FormRow_Separator());
                    out.println(FormRow_ComboBox (PARM_ACCT_PRIVLABEL , editSysAdmin , i18n.getString("SysAdminAccounts.privateLabelName","PrivateLabel Name")+":"      , acctPrivLbl , privLblList, "", 40));
                    out.println(FormRow_ComboBox (PARM_ACCT_RG_MODE   , editSysAdmin , i18n.getString("SysAdminAccounts.geocoderMode","Geocoder Mode:")                 , geocoderMode, _rgList, null, 40));
                    if (Account.SupportsBorderCrossing()) {
                        out.println(FormRow_ComboBox (PARM_ACCT_IS_BCROSS, editSysAdmin, i18n.getString("SysAdminAccounts.isBorderCross","Enable Border Crossing:")     , isBCross    , ComboMap.getYesNoMap(locale), "", 40));
                    }
                    if (isSysAdmin) {
                    out.println(FormRow_TextField(PARM_ACCT_DCS_PROPS_ID, editSysAdmin, i18n.getString("SysAdminAccounts.dcsPropertiesID","DCS Properties ID")+":"        , dcsPropsID  , 40, 32));
                    }
                    ...
                    if (notesOK) {
                        ...
                        out.println(FormRow_Separator());
                        out.println(FormRow_TextArea(PARM_ACCT_NOTES, _editAccount, i18n.getString("SysAdminAccounts.notes" ,"General Notes")+":", noteText, 8, 70));
                    }
                    
                    HttpServletRequest request = reqState.getHttpServletRequest();
                    
                    String accountID=    AttributeTools.getRequestString(request, "s_account", "");
                    String allow ="";
                    String btnXem =AttributeTools.getRequestString(request, "a_subview", "");
                   
                    if(btnXem.equals(i18n.getString("SysAdminAccounts.view","View")))
                    {
                    	allow="disabled";
                    }
                    out.print("<tr><td align='right' valign='top'><br/><hr/><br/>Trang không được xem<br/>(Camera được xem):</td><td><br/><hr/><br/><div style='border: 1px solid white; height:120px; width:510px; overflow-x: hidden; overflow-y: scroll;'>"+load_Trang( accountID,allow)+"</div></td></tr>");
                    ...
                    if (_editAccount) {
                        out.write("<input type='submit' class='button' name='"+PARM_SUBMIT_CHG+"' value='"+i18n.getString("SysAdminAccounts.change","Change")+"'>\n");
                        out.write("<span style='padding-left:10px'>&nbsp;</span>\n");
                        out.write("<input type='button' class='button' name='"+PARM_BUTTON_CANCEL+"' value='"+i18n.getString("SysAdminAccounts.cancel","Cancel")+"' onclick=\"javascript:openURL('"+editURL+"','_top');\">\n");
                       
                        
                    } else {
                        out.write("<input type='button' class='button' name='"+PARM_BUTTON_BACK+"' value='"+i18n.getString("SysAdminAccounts.back","Back")+"' onclick=\"javascript:openURL('"+editURL+"','_top');\">\n");
                    }
                    out.write("</form>\n");
                ...
                String  thayDoi  = AttributeTools.getRequestString(request,"a_subchg", "");
                String accountID=    AttributeTools.getRequestString(request, "s_account", "");
              if (thayDoi.equals("Change"))
              {
            	  
            	   String[] checkbox;
                   checkbox =request.getParameterValues("checkTrang");
                   DBCamera objcmr = new  DBCamera();
                   objcmr.deleteTblQuyen(accountID);
                   	if(checkbox!=null)
                   	{
                   		for(int i=0;i<checkbox.length;i++)
                   			
                   		{
                   				objcmr.insertTblQuyen(accountID, checkbox[i]);
