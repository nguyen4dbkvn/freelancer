...
public class EventUtil
{
    ...
    */
    ...
    // minhnv
    private static String Minhnv_YMDtoDMY(String date)
    {
    ...
    /*public String laiXeHienTai(String device,String acc) 
    {
    	String chuoi ="";
    	DBCamera db = new DBCamera();
    	List<String[]> result = db.select_LaiXeHienTai(device, acc);  
    	if(result.size()>0)
    	{
    		for(String[] rs:result)
    		{
				chuoi = chuoi
						+ rs[2]
						+ "<br/><b>Số điện thoại:</b>"
						+ rs[1]
						+ "<br/><b>Giấy phép lái xe:</b>"
						+ rs[4]
						+ "<br/><b>Ngày hết hạn bằng lái: </b>"
						+ Minhnv_YMDtoDMY(FormatDayNumber(Long.parseLong(rs[7])));
					
				if ((!rs[6].isEmpty() && (rs[6].compareTo("")!=0)) && (!rs[5].isEmpty() && (rs[5].compareTo("")!=0))){
					chuoi = chuoi + "<br/><b>Số khung: </b>" + rs[5]
							+ "<br/><b>Số máy: </b>" + rs[6]; 
				}
    	...
    	else
    	{
    		...
    		try
    		{
    		    list1= db.Laixehientai1(device,acc);
    		}
    		catch (Exception e) {
    		...
    		if(list1.size()>0)
    		{
	    		...
	    		{
//					chuoi = chuoi+ rs1[2]+"<br/><b>Số điện thoại:</b>"+rs1[1]+"<br/><b>Giấy phép lái xe:</b>"+rs1[4]+"<br/><b>Số VIN:</b>" +rs1[5];
					chuoi = chuoi+ rs1[2]+"<br/><b>Số điện thoại:</b>"+rs1[1]+"<br/><b>Giấy phép lái xe:</b>"+rs1[4];
	    		}
    		}
    		else
    		chuoi ="Không xác định";
    	}
    	return chuoi;
    ...
    public String laiXeHienTai(String device, String acc) {
		...
		DBCamera db = new DBCamera();
		try {
			List<String[]> result = db.Laixehientai2(device, acc);

			if (result.size() > 0) {
				for (String[] rs : result) {

					chuoi = chuoi + rs[2] + "<br/><b>Số điện thoại: </b>" + rs[1] + "<br/><b>Giấy phép lái xe: </b>"
							+ rs[4] + "<br/><b>Loại xe: </b>" + rs[6];
				}
			} // mr viet edit
			else {
				chuoi = "Không xác định";
    ...
    public String tglx(String dev,String acc)
    {
    	int tt[] = new int[3000];
    	int tg[] = new int[3000];
    	String chuoi ="",from="",date="";
    	DBCamera db = new DBCamera();
    	ResultSet rs1 =db.ThoiGian(acc, dev) ;
    	...
    	date = from.split(" ")[0];
    	int i =0,k=0, m=0,chay=0,lxlt=0,lxtn=0;
    	
    	try{
    		ResultSet rs =db.thoiGianLaiXe(acc, dev, date);
    		
    		while(rs.next())
    		{
    			
    			 tt[i] = Integer.parseInt(rs.getString(1));
    			 tg[i]=Integer.parseInt(rs.getString(2));
    			 i++;
    	...
    	for(int j =0;j<i;j++)
    	{
    		...
    		if((tg[k]-tg[k+m+1]>900)&&(tt[k+m+1]!=61472))
    		{
    			if(chay==0)
    			{
    				lxlt=tg[chay]-tg[k];
    			...
    			lxtn = lxtn + (tg[chay]-tg[k]);
    			//chuoi=Integer.toString(tg[0]-tg[k]) +"Thời điểm bắt đầu lái:" +Integer.toString(tg[k])+"/"+Integer.toString(m);
    			chay = k+m+1;
    			//break;
    		}
    	...
    	// khi bản ghi đầu tiên của ngày không phải là bản ghi dừng
    	if((chay!=i))
    	lxtn = lxtn +tg[chay]-tg[i-1];
    	
    	chuoi= chuoi +"<b>TG lái xe liên tục:</b>"+ doiGio(lxlt)+"<br/><b>TG lái xe trong ngày:</b>"+doiGio(lxtn);
    	}else
    	{chuoi= chuoi +"<b>TG lái xe liên tục:</b>0<br/><b>TG lái xe trong ngày:</b>0";}
    	return chuoi;
    ...
    public String doiGio(int tg)
    {
        ...
        return chuoi =chuoi+h+" Giờ "+m+" Phút "+s+" Giây ";
  ...
  //public  static final String  CSV_SEPARATOR                  = "|";
    ...
    public String getParseMapEventJS(boolean isFleet, Locale locale, char csvSep)
    {
      ...
      //js.append("    //if (this.address == '') { this.address = '&nbsp;'; }\n"); // fill with space, so field isn't blank
        ...
        js.append("    this.lastImageLink   = (fld.length > 17)? decodeUnicode(fld[17].trim()) : '';\n");
        /*Minhnv get last image for device - END */ 
       
        js.append("    this.deviceId   = (fld.length > 18)? decodeUnicode(fld[18]) : '';\n");//tanvm: add deviceID
        js.append("    this.statusLastingTime   = (fld.length > 19)? parseInt(fld[19]) : 0;\n");//tanvm: add status lasting time
        js.append("    this.Cua   = (fld.length > 20)? parseInt(fld[20]) : 0;\n");//hienntd: add Cua
        js.append("    this.DieuHoa   = (fld.length > 21)? parseInt(fld[21]) : 0;\n");//hienntd: add DieuHoa
        /*Minhnv get last image for Fuel - BEGIN */
        js.append("    this.LastFuel   = (fld.length > 22)? decodeUnicode(fld[22].trim()) : '';\n");
        js.append("    this.LaiXeHienTai   = (fld.length > 23)? decodeUnicode(fld[23].trim()) : '';\n");
        js.append("    this.thoiGianLaiXe   = (fld.length > 24)? decodeUnicode(fld[24]) : '';\n");
        js.append("    this.accountID   = (fld.length > 25)? decodeUnicode(fld[25]) : '';\n");
         /*Minhnv get last image for Fuel - END */ 
//      //hienntd add statuscode
//        js.append("    this.statuscode = (fld.length > 23)? parseInt(fld[23]) : 0;\n");        // status code
//        
        
        js.append("    if (fld.length > 26) {\n");
        js.append("        this.optDesc = new Array();\n");
        js.append("        for (var i = 26; i < fld.length; i++) {\n");
    ...
    {
        // ID   |DeviceDesc|Epoch     |Date      |Time    |Timezone|StatusCodeDesc|Icon|Latitude |Longitude  |Accuracy|Satellites|SpeedKPH|Heading|Altitude|Odometer|Address           |extra...
        // demo2|New Device|1268394640|2010/03/12|05:50:40|CST     |InMotion      |19  |37.785248|-121.307275|0.0     |0         |87.2    |227.7  |7       |16124.9 |"I-5, Lathrop, CA"
       ...
       //long startTime=System.currentTimeMillis()/1000L;
        ...
        if(edp.getStatusCode()==61718)
         sb.append("Dừng, đang hoạt động");
        else
        ...
        sb.append(StringTools.quoteString(edp.getAddress()));
        sb.append(csvSep);
        
        //[17]
        /*Minhnv get last image for device - BEGIN */
        if (isFleet)
        	sb.append(Device.getLastImageLink(edp.getDeviceID()));
        else
        	sb.append("");
        sb.append(csvSep);
        /*Minhnv get last image for device - END */
        ...
        //tanvm: deviceID [18]
        try {
        	Device dv = Device.getDevice(Account.getAccount(edp.getAccountID()), edp.getDeviceID());        	
        	sb.append(EventUtil._encUnicode(dv.getDeviceID(),csvSep));
        	sb.append(csvSep);
        }catch (Exception ex){
        	
        }
        /* [19] StatusLastingTime */
        try {
    	DBCamera objcmr = new  DBCamera();
    	int starus= objcmr.StaticSelectstatus(edp.getDeviceID());
    	long statusLastingTime=0;
    	if(starus==1)
    	{
    		if(edp.getStatusLastingTime()>0)
    		{
    		statusLastingTime=System.currentTimeMillis()/1000-edp.getTimestamp()+edp.getStatusLastingTime();
    		}
    	}
    	else {
    		statusLastingTime = edp.getStatusLastingTime();
    	}
        sb.append(statusLastingTime);
        sb.append(csvSep);
        }catch (Exception ex){
        ...
        if(isFleet)
        sb.append(Device.getLastFuel(edp.getDeviceID(),edp.getAccountID()));
        else
        	sb.append("");
        sb.append(csvSep);
        //[23] lai xe hien tai
        ...
        if(isFleet)
            sb.append(laiXeHienTai(edp.getDeviceID(),edp.getAccountID()));
            else
            	sb.append("laiXeHienTai");
            sb.append(csvSep);
            //[24] lai xe hien tai
            if(isFleet)
                sb.append(tglx(edp.getDeviceID(),edp.getAccountID()));
                else
                	sb.append("ThoiGianLaiXe");
                sb.append(csvSep);
        
                
//        // hienntd add status code

                
        sb.append(edp.getAccountID());
        sb.append(csvSep);
//        
        /* [22+] other fields? (unicode encoded) */
        if (EventUtil.optionalEventFieldHandler != null) {
            int optFieldCount = EventUtil.optionalEventFieldHandler.getOptionalEventFieldCount(isFleet);
            for (int i = 0; i < optFieldCount; i++) {
        ...
        //hienntd add
        /* [24] Driver 
        String BadgeID = "", DriverName = "", DriverPhone = "";
        try {
        Device dv = Device.getDevice(Account.getAccount(edp.getAccountID()), edp.getDeviceID());  
         Driver drv = Driver.getDriver(Account.getAccount(edp.getAccountID()), dv.getDriverID());
         BadgeID = drv.getBadgeID();
         DriverName = drv.getDescription();
         DriverPhone = drv.getContactPhone();
         sb.append(EventUtil._encUnicode(DriverName +" "+DriverPhone,csvSep));
     	sb.append(csvSep);
        } catch (Exception ex) {             
        }
		
        /* return CSV record */
        //System.out.println("Time formatEvents: "+(System.currentTimeMillis()/1000L-startTime) );
        return sb.toString();
    ...
    //	System.out.println("Next 3.1.1");
     ...
     //   System.out.println("Next 3.1.4");
        ...
        this.write(pwout, XMLTools.CDATA(isSoapRequest, // see "formatMapEvent"
            "id|desc|epoch|date|time|tmz|status|icon|lat|lon|acc|sats|kph|heading|alt|odomkm|addr|accountID|" // |extra...
            ));
        this.write(pwout, XMLTools.endTAG(isSoapRequest,TAG_DataColumns,true));
       // System.out.println("Start get EventMaps");
        // <DataSet type="poi">
     ...
     //   long startTime=System.currentTimeMillis()/1000L;
        ...
        boolean rtn = this._writeMapEvents_xml(
            ...
            csvSep,
            minProximityM);
      //  System.out.println("Time _writeMapEvents_xml: "+(System.currentTimeMillis()/1000L-startTime) );
        /* XML footer */
        this.write(pwout, PFX1);
    ...
    private boolean _writeMapEvents_xml(
        ...
        throws IOException
    	{
    	//System.out.println("Next 3.1.5.1");
    	long startTime=System.currentTimeMillis()/1000L;
        /* valid EventDataProvider? */
        ...
        String   textColor    = "";
        int      evNdx        = 0;
     //   System.out.println("Next 3.1.5.2");
        //System.out.println("Size of list "+edp.length);
        for (int i = 0; i < edp.length; i++) {
            String thisDevID = edp[i].getDeviceID();
            //Print.logInfo(i + ") Event: " + thisDevID);
       //     System.out.println("Next 3.1.5.2.1");
            /* first device? */
        ...
        //    System.out.println("Next 3.1.5.2.2");
            /* event index */
       ...
       //     System.out.println("Next 3.1.5.2.3");
      ...
      //      System.out.println("Next 3.1.5.2.4");
      ...
      //      System.out.println("Next 3.1.5.2.5");
      ...
      //  System.out.println("Time Process getData: "+(System.currentTimeMillis()/1000L-startTime) );
        return true;
