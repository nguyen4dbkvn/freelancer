...
public class BaoCaoBatTatCanCau extends WebPageAdaptor implements Constants {
	...
	public String getMenuDescription(RequestProperties reqState, String parentMenuName) {
		...
		I18N i18n = privLabel.getI18N(BaoCaoBatTatCanCau.class);
		return super._getMenuDescription(reqState, "Báo cáo bật tắt điều khiển cần cẩu");
	}

	public String getMenuHelp(RequestProperties reqState, String parentMenuName) {
		PrivateLabel privLabel = reqState.getPrivateLabel();
		I18N i18n = privLabel.getI18N(BaoCaoBatTatCanCau.class);
		return super._getMenuHelp(reqState, "Báo cáo bật tắt điều khiển cần cẩu");
	}
	...
	public String getNavigationDescription(RequestProperties reqState) {
		PrivateLabel privLabel = reqState.getPrivateLabel();
		I18N i18n = privLabel.getI18N(BaoCaoBatTatCanCau.class);
		return super._getNavigationDescription(reqState, "Báo cáo bật tắt điều khiển cần cẩu");
	}
	...
	public String LoadJobs(String accountID, String deviceID, String tuNgay, String denNgay, String timezone, I18N i18n)
			throws IOException {
		String strscr = "";
		try {
			DBCamera db = new DBCamera();
			ArrayList<TinHieuCau> list = db.TinHieuCau(accountID, deviceID, tuNgay, denNgay, timezone);
			strscr = strscr + "	<table  id='myTable' width='100%' class='adminSelectTable_sortable' cellspacing='1' >"
					+ "			<thead><tr align='center'>"
					+ "				<th class = 'adminTableHeaderCol_sort' width='30px'>"
					+ i18n.getString("MayXucLiveReport.LiveNo", "STT") + "</th>"
					+ "				<th class = 'adminTableHeaderCol_sort' width='80px'>"
					+ i18n.getString("MayXucLiveReport.DriverID", "Xe") + "</th>"
					+ "				<th class = 'adminTableHeaderCol_sort' width='120px'>"
					+ i18n.getString("MayXucLiveReport.StartTime", "Thời gian bắt đầu") + "</th>"
					+ "				<th class = 'adminTableHeaderCol_sort' width='120px'>"
					+ i18n.getString("MayXucLiveReport.EndTime", "Thời gian kết thúc") + "</th>"
					+ "				<th class = 'adminTableHeaderCol_sort' width='80px'>"
					+ i18n.getString("MayXucLiveReport.DriverID", "Trạng thái") + "</th>"
					+ "				<th class = 'adminTableHeaderCol_sort' width='100px'>"
					+ i18n.getString("MayXucLiveReport.LiveTime", "Thời gian làm việc") + "</th>"
					+ "				<th class = 'adminTableHeaderCol_sort' width=''>"
					+ i18n.getString("MayXucLiveReport.StartAddress", "Địa chỉ bắt đầu") + "</th>"
					+ "				<th class = 'adminTableHeaderCol_sort' >"
					+ i18n.getString("MayXucLiveReport.EndAddress", "Địa chỉ kết thúc") + "</th>"
					+ "				</tr></thead>" + "		<tbody>";
			boolean odd = true;
			int tong = 0, status = 0, startTime = 0, count = 0;

			String start = "", end = "";

			for (int i = 0; i < list.size(); i++) {
				if (status != list.get(i).DieuHoa) {
					// ban ghi dau tien
					if (start.equals("")) {
						start = NCRToUnicode(list.get(i).address);
						startTime = list.get(i).timestamp;
					} else {
						if (DongMo(status).equals("Bật")) {
							count++;
							String strOdd = (odd) ? " class='adminTableBodyRowOdd' " : "class='adminTableBodyRowEven' ";
							odd = !odd;
							strscr = strscr + "<tr " + strOdd + " ><td>" + count + "</td><td>" + list.get(i).deviceID
									+ "</td><td>" + ConvertFromEpoch(startTime, timezone) + "</td><td>"
									+ ConvertFromEpoch(list.get(i).timestamp, timezone) + "</td><td>" + DongMo(status)
									+ "</td><td>" + formatElapsedTime(list.get(i).timestamp - startTime)
									+ "</td><td><span style='margin:0 2px'>" + start
									+ "</span></td><td><span style='margin:0 2px'>" + list.get(i).address
									+ "</span></td>" + "</tr>";
							// danh dau lai
							tong += list.get(i).timestamp - startTime;
						}
						start = NCRToUnicode(list.get(i).address);
						startTime = list.get(i).timestamp;
						status = list.get(i).DieuHoa;
					}
				}
			}
			String strOdd = (odd) ? " class='adminTableBodyRowOdd' " : "class='adminTableBodyRowEven' ";
			if (list.size() > 0)
				strscr = strscr
						+ "<tr class='adminTableBodyRowOdd'><td style='font-weight:bold'>Tổng</td><td></td><td></td><td></td><td></td><td style='font-weight:bold;'>"
						+ formatElapsedTime(tong) + "</td><td></td><td></td></tr>";
	...
	public String LoadVuotTocDo(String Account, String tuNgay, String denNgay, String Device, String timezone,
			RequestProperties reqState, PrivateLabel privLabel) throws IOException {
		int dem = 0;
		...
		try {
			...
			chuoi = "<tr style='height:30px;color:black'><td colspan='5' align='left'><span style='margin-left:25px;'><b>"
					+ Device + "</b></span></td></tr>";
			EventData ed[] = objcmr.GetReportdongmo(tuNgay, denNgay, Device, timezone, Account);
			int status = 0;
			for (int k = 0; k < ed.length; k++) {
				if (status != ed[k].getDieuHoa()) {
					...
					String day = ConvertFromEpoch(ed[k].getTimestamp(), timezone);
					chuoi1 = chuoi1 + "<tr class =" + css + "><td>" + record + "</td><td>" + day + "</td><td>"
							+ DongMo(ed[k].getDieuHoa()) + "</td><td>" + round(ed[k].getSpeedKPH(), 2) + "</td><td>"
	...
	public void writePage(final RequestProperties reqState,
		...
		final I18N i18n = privLabel.getI18N(BaoCaoBatTatCanCau.class);
		...
		if (excel != "") {
			if (excel.equals("Export Excel")) {
				...
				ngay.createCell((short) 2).setCellValue(datefrom.replace("00:0", " "));
				HSSFCell cdenNgay = ngay.createCell((short) 3);

				cdenNgay.setCellStyle(csNgay);
				cdenNgay.setCellValue("Đến ngày");
				ngay.createCell((short) 4).setCellValue(dateto.replace("23:59", " "));
				...
				try {
					...
					EventData ed[] = objcmr.GetReportdongmo(datefrom, dateto, contentall, currAcct.getTimeZone(),
					...
					for (int i = 0; i < ed.length; i++) {
						...
						String day = ConvertFromEpoch(ed[i].getTimestamp(), currAcct.getTimeZone());
		...
		HTMLOutput HTML_CSS = new HTMLOutput() {
			public void write(PrintWriter out) throws IOException {
				String cssDir = BaoCaoBatTatCanCau.this.getCssDirectory();
		...
		HTMLOutput HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
			public void write(PrintWriter out) throws IOException {
				...
				String frameTitle = i18n.getString("Camera.PageTitle", "BaoCaoDongMo");
				HttpServletRequest request = reqState.getHttpServletRequest();
				String contentall = "";
				...
				String sql = LoadJobs(currAcct.getAccountID(), contentall, datefrom, dateto, currAcct.getTimeZone(),
						i18n);
				Date now = new Date();
				SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
				String d = sdf.format(now);
				...
				out.println("<hr/>");
				out.println("<form name='baocaodongmo' method='post' action='" + chgURL + "' target='_self'>\n");
