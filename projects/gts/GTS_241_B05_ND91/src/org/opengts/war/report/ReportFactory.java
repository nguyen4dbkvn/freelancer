...
public class ReportFactory {
	...
	public static final String REPORT_TYPE_DEVICE_QLSIM = "device.qlsim";

	public static final String REPORT_TYPES[] = new String[] {
			REPORT_TYPE_DEVICE_DETAIL, REPORT_TYPE_DEVICE_SUMMARY,
			REPORT_TYPE_FLEET_DETAIL, REPORT_TYPE_FLEET_SUMMARY,
			REPORT_TYPE_DRIVER_PERFORM, REPORT_TYPE_IFTA_DETAIL,
			REPORT_TYPE_IFTA_SUMMARY, REPORT_TYPE_SYSADMIN_SUMMARY };

	public static String getReportTypeDescription(RequestProperties reqState,
			String rptType) {
		PrivateLabel privLabel = reqState.getPrivateLabel();

		/* get 'report.xml' description */
		String desc = ReportFactory.getReportTypeDescription(rptType,
				reqState.getLocale());
		...
		if (rptType.equalsIgnoreCase(REPORT_TYPE_DEVICE_DETAIL)) {
			return i18n.getString("ReportFactory.deviceDetailReports",
					"{0} Detail Reports:", devTitles);
		} else if (rptType.equalsIgnoreCase(REPORT_TYPE_DEVICE_SUMMARY)) {
			return i18n.getString("ReportFactory.deviceSummaryReports",
					"{0} Summary Reports:", devTitles);
		} else if (rptType.equalsIgnoreCase(REPORT_TYPE_FLEET_DETAIL)) {
			return i18n.getString("ReportFactory.fleetDetailReports",
					"{0} Detail Reports:", grpTitles);
		} else if (rptType.equalsIgnoreCase(REPORT_TYPE_FLEET_SUMMARY)) {
			return i18n.getString("ReportFactory.fleetSummaryReports",
					"{0} Summary Reports:", grpTitles);
		} else if (rptType.equalsIgnoreCase(REPORT_TYPE_DRIVER_PERFORM)) {
			return i18n.getString("ReportFactory.driverPerformanceReports",
					"Driver Performance Reports:");
		} else if (rptType.equalsIgnoreCase(REPORT_TYPE_IFTA_DETAIL)) {
			return i18n.getString("ReportFactory.iftaReports",
					"I.F.T.A. Reports:");
		} else if (rptType.equalsIgnoreCase(REPORT_TYPE_IFTA_SUMMARY)) {
			return i18n.getString("ReportFactory.iftaSummaryReports",
					"I.F.T.A. Summary Reports:");
		} else if (rptType.equalsIgnoreCase(REPORT_TYPE_SYSADMIN_SUMMARY)) {
			return i18n.getString("ReportFactory.sysadminReports",
	...
	public static class ReportFactoryVars {
		...
		public ReportFactoryVars(File xmlFile) {
			this.xmlFile = (xmlFile != null) ? xmlFile : ReportFactory
					._getReportXMLFile();
			this.reset();
		}
		...
		public void addReportFactory(ReportFactory rf) throws ReportException {
			if (rf != null) {
				...
				if (ReportFactory._getReportFactory(name) != null) {
					throw new ReportException("Report name already exists: "
							+ name);
				}
		...
		public Collection<ReportFactory> getReportFactories() {
			return (this.rptFactoryMap != null) ? this.rptFactoryMap.values()
					: null;
		}
	...
	private static String getNodeText(Node root, String repNewline) {
		...
		if (root != null) {
			...
			for (int i = 0; i < list.getLength(); i++) {
				Node n = list.item(i);
				if (n.getNodeType() == Node.CDATA_SECTION_NODE) { // CDATA
																	// Section
					text.append(n.getNodeValue());
				} else if (n.getNodeType() == Node.TEXT_NODE) {
					text.append(n.getNodeValue());
				} else {
					// Print.logWarn("Unrecognized node type: " +
					// n.getNodeType());
				}
			}
		}
	...
	private static int _loadReportDefinitionXML(ReportFactoryVars rfv) {
		...
		if (!reportDef.getTagName().equalsIgnoreCase(TAG_ReportDefinition)) {
			Print.logError("[" + xmlFile + "] Invalid root tag ID: "
					+ reportDef.getTagName());
			rfv.setHasParsingErrors();
			return count;
		}
			...
			for (int pn = 0; pn < propertiesNodes.getLength(); pn++) {
				...
				for (int p = 0; p < propNodes.getLength(); p++) {
					...
					if (!StringTools.isBlank(propKey)) {
						...
						Print.logError("[" + xmlFile
								+ "] Report Property 'key' is blank");
						rfv.setHasParsingErrors();
					}
				}
			}
			// override with properties defined in the runtime config files
			RTProperties globalProps = RTConfig.getProperties(
					PROP_ReportDefinition_, false);
			for (Object gk : globalProps.getPropertyKeys()) {
				if ((gk instanceof String)
						&& ((String) gk).startsWith(PROP_ReportDefinition_)) {
					Object gv = globalProps.getProperty(gk, null);
					if (gv instanceof String) {
						String propKey = ((String) gk)
								.substring(PROP_ReportDefinition_.length());
		...
		NodeList defaultStyleNodes = XMLTools.getChildElements(reportDef,
				TAG_DefaultStyle);
		...
		ReportLayout
				.setDefaultStyleSheet(ReportFactory.reformatStyle(dftStyle));
		// Print.logInfo("DefaultStyle: \n" +
		// ReportLayout.getDefaultStyleSheet());

		/* parse ReportLayout */
		NodeList layoutList = XMLTools.getChildElements(reportDef,
				TAG_ReportLayout);
		for (int rl = 0; rl < layoutList.getLength(); rl++) {
			...
			Boolean isOptional = StringTools.parseBoolean(
					reportLayout.getAttribute(ATTR_optional), false);

			/* parse DateFormat */
			String dateFmt = null;
			NodeList dateFmtNodes = XMLTools.getChildElements(reportLayout,
					TAG_DateFormat);
			...
			/* parse TimeFormat */
			String timeFmt = null;
			NodeList timeFmtNodes = XMLTools.getChildElements(reportLayout,
					TAG_TimeFormat);
			...
			NodeList layoutStyleNodes = XMLTools.getChildElements(reportLayout,
					TAG_LayoutStyle);
			for (int dsl = 0; dsl < layoutStyleNodes.getLength(); dsl++) {
				Element lsTag = (Element) layoutStyleNodes.item(dsl);
				// CSS file
				String cssFile = XMLTools.getAttribute(lsTag, ATTR_cssFile,
						null);
				if (!StringTools.isBlank(cssFile)) {
			...
			try {
				// invoke static method "getReportLayout", this returns the
				// layout singleton instance
				MethodAction ma = new MethodAction(layoutClass,
						"getReportLayout");
				ReportLayout rptLayout = (ReportLayout) ma.invoke(); // may
																		// throw
																		// ClassNotFoundException,
																		// etc
				rptLayout.setDateTimeFormat(dateFmt, timeFmt);
				rptLayout.setCSSFiles(cssFiles);
				rptLayout.setStyleSheet(ReportFactory
						.reformatStyle(layoutStyle));
			} catch (ClassNotFoundException cnfe) {
				if (!IGNORE_MISSING_REPORTS && !isOptional) {
					Print.logError("[" + xmlFile
							+ "] ReportLayout class not found: " + layoutClass);
					rfv.setHasParsingErrors();
				} else if (RTConfig.isDebugMode()) {
					Print.logWarn("Optional ReportLayout class not found: "
							+ layoutClass);
					rfv.setHasParsingErrors();
				} else {
					Print.logDebug("Ignoring Optional ReportLayout: "
							+ layoutClass);
				}
			} catch (NoSuchMethodException nsme) {
				Print.logError("[" + xmlFile
						+ "] ReportLayout static method not found: "
						+ layoutClass + ".getReportLayout()");
				rfv.setHasParsingErrors();
			} catch (Throwable t) {
				Print.logException("[" + xmlFile
						+ "] Exception while initializing ReportLayout: "
						+ layoutClass, t);
				rfv.setHasParsingErrors();
			}
		...
		for (int r = 0; r < reportList.getLength(); r++) {
			...
			boolean rptOptional = XMLTools.getAttributeBoolean(report,
					ATTR_optional, false);
			boolean rptSysAdminOnly = XMLTools.getAttributeBoolean(report,
					ATTR_sysAdminOnly, false);
			boolean rptTableSortable = XMLTools.getAttributeBoolean(report,
					ATTR_sortable, false);
			...
			for (int c = 0; c < attrList.getLength(); c++) {
				...
				if (attrName.equalsIgnoreCase(TAG_MenuDescription)) {
					...
					rptMenu = ReportFactory.parseI18N(i18nPkgName, i18nKey,
							textDft);
					// Print.logInfo("  MenuDescription: " + rptMenu);
				} else if (attrName.equalsIgnoreCase(TAG_Title)) {
					String i18nKey = attrElem.getAttribute(ATTR_i18n);
					String textDft = ReportFactory.getNodeText(attrElem, "\\n");
					rptTitle = ReportFactory.parseI18N(i18nPkgName, i18nKey,
							textDft);
					// Print.logInfo("  Title: " + rptTitle);
				} else if (attrName.equalsIgnoreCase(TAG_Subtitle)) {
					String i18nKey = attrElem.getAttribute(ATTR_i18n);
					String textDft = ReportFactory.getNodeText(attrElem, "\\n");
					rptSubt = ReportFactory.parseI18N(i18nPkgName, i18nKey,
							textDft);
					// Print.logInfo("  Subtitle: " + rptSubt);
				} else if ((rptCols == null)
						&& attrName.equalsIgnoreCase(TAG_SimpleColumns)) {
					String columns = ReportFactory.getNodeText(attrElem, null);
					String cols[] = StringTools
							.parseString(columns, ", \t\r\n");
					...
					for (int i = 0; i < cols.length; i++) {
						...
						if (!colName.equals("")) {
							...
							ReportColumn rc = new ReportColumn(colKey, colArg,
									null);
					...
					rptCols = colList.toArray(new ReportColumn[colList.size()]);
						...
						&& attrName.equalsIgnoreCase(TAG_Columns)) {
					java.util.List<ReportColumn> colList = new Vector<ReportColumn>();
					NodeList columnList = XMLTools.getChildElements(attrElem,
							TAG_Column);
					for (int z = 0; z < columnList.getLength(); z++) {
						Element column = (Element) columnList.item(z);
						String colName = XMLTools.getAttribute(column,
								ATTR_name, column.getAttribute(ATTR_key));
						String ifTrue = XMLTools.getAttribute(column,
								ATTR_ifTrue, null);
						String ifFalse = XMLTools.getAttribute(column,
								ATTR_ifFalse, null);
						if (!StringTools.isBlank(ifTrue)
								&& (rptDefProps.getBoolean(ifTrue, true) != true)) {
							// property is explicitly set to 'false' ... ignore
							// column
							Print.logDebug("Ignoring column [" + ifTrue
									+ "==true]: " + rptName + "." + colName);
						} else if (!StringTools.isBlank(ifFalse)
								&& (rptDefProps.getBoolean(ifFalse, true) != false)) {
							// property is 'true' (ie. not 'false') ... ignore
							// column
							Print.logDebug("Ignoring column [" + ifFalse
									+ "==false]: " + rptName + "." + colName);
						} else {
							String colArg = XMLTools.getAttribute(column,
									ATTR_arg, "");
							boolean colSort = XMLTools.getAttributeBoolean(
									column, ATTR_sortable, true);
							String titlsStr = ReportFactory.getNodeText(column,
									"\\n");
							String blankStr = XMLTools.getAttribute(column,
									ATTR_blankFill, null);
							I18N.Text colTitle = null;
							if (!StringTools.isBlank(titlsStr)) {
								String i18nKey = XMLTools.getAttribute(column,
										ATTR_i18n, null);
								colTitle = ReportFactory.parseI18N(i18nPkgName,
										i18nKey, titlsStr);
							}
							ReportColumn rc = new ReportColumn(colName, colArg,
									colTitle);
					...
					rptCols = colList.toArray(new ReportColumn[colList.size()]);
				} else if ((rptOptMap == null)
						&& attrName.equalsIgnoreCase(TAG_Options)) {
					rptOptMap = new OrderedMap<String, ReportOption>();
					NodeList optionList = XMLTools.getChildElements(attrElem,
							TAG_Option);
					for (int z = 0; z < optionList.getLength(); z++) {
						Element option = (Element) optionList.item(z);
						String optName = XMLTools.getAttribute(option,
								ATTR_name, "");
						String ifTrue = XMLTools.getAttribute(option,
								ATTR_ifTrue, null);
						String ifFalse = XMLTools.getAttribute(option,
								ATTR_ifFalse, null);
						if (StringTools.isBlank(optName)) {
							Print.logError("[" + xmlFile
									+ "] Missing Option name");
							rfv.setHasParsingErrors();
						} else if (!StringTools.isBlank(ifTrue)
								&& (rptDefProps.getBoolean(ifTrue, true) != true)) {
							// property is explicitly set to 'false' ... ignore
							// option
							Print.logDebug("Ignoring option [" + ifTrue
									+ "==true]: " + rptName + "." + optName);
						} else if (!StringTools.isBlank(ifFalse)
								&& (rptDefProps.getBoolean(ifFalse, true) != false)) {
							// property is 'true' (ie. not 'false') ... ignore
							// option
							Print.logDebug("Ignoring option [" + ifFalse
									+ "==false]: " + rptName + "." + optName);
						} else {
							...
							for (int zz = 0; zz < optChildList.getLength(); zz++) {
								...
								if (!(optChildNode instanceof Element)) {
									continue;
								}
								String optChildName = optChildNode
										.getNodeName();
								Element optChildElem = (Element) optChildNode;
								if (optChildName
										.equalsIgnoreCase(TAG_Description)) {
									String i18nKey = optChildElem
											.getAttribute(ATTR_i18n);
									String textDft = ReportFactory.getNodeText(
											optChildElem, "\\n");
									rptOpt.setDescription(ReportFactory
											.parseI18N(i18nPkgName, i18nKey,
													textDft));
								} else if (optChildName
										.equalsIgnoreCase(TAG_Property)) { // Option
																			// Property
									String propKey = optChildElem
											.getAttribute(ATTR_key);
									if (!StringTools.isBlank(propKey)) {
										String propVal = ReportFactory
												.getNodeText(optChildElem,
														"\\n");
										propVal = rptDefProps
												.insertKeyValues(propVal);
										rptOpt.setValue(propKey, propVal);
									} else {
										Print.logError("["
												+ xmlFile
												+ "] Option Property 'key' is blank: "
												+ optName);
										rfv.setHasParsingErrors();
									}
								} else {
						...
						&& attrName.equalsIgnoreCase(TAG_Constraints)) {
					rptRC = parseReportConstraintsXML(rfv, attrElem,
							rptDefProps);
					// Print.logInfo("ReportConstraints: ...");
				} else if ((rptIconSel == null)
						&& attrName.equalsIgnoreCase(TAG_MapIconSelector)) {
					...
					if (ruleFact == null) {
					...
					} else if (StringTools.isBlank(rfName)
							|| rfName.equalsIgnoreCase(ruleFact.getName())) {
						rptIconSel = ReportFactory.getNodeText(attrElem, " "); // 'rptIconSel'
																				// is
																				// saved
																				// later
						if (!ruleFact.checkSelectorSyntax(rptIconSel)) {
							Print.logError("[" + xmlFile
									+ "] Invalid MapIconSelector syntax: "
									+ rptIconSel + " [" + ruleFact.getName()
									+ "]");
							rfv.setHasParsingErrors();
						} else {
						...
						// Print.logWarn("[" +xmlFile +
						// "] Ignoring MapIconSelector for RuleFactory '"+rfName+"'");
					}
				} else if (attrName.equalsIgnoreCase(TAG_Property)) { // Report
																		// Property
					String propKey = attrElem.getAttribute(ATTR_key);
					if (!StringTools.isBlank(propKey)) {
						// String globalKey = PROP_ReportDefinition_ + rptName +
						// "." + propKey;
						// String propVal =
						// rptDefProps.getString(globalKey,null);
						...
						// Print.logInfo("Report '%s' property: %s ==> %s",
						// rptName, propKey, propVal);
					} else {
						Print.logError("[" + xmlFile
								+ "] Report Property 'key' is blank: "
								+ rptName);
					...
					Print.logError("[" + xmlFile + "] Unrecognized tag name: "
							+ attrName);
					rfv.setHasParsingErrors();
				}
			...
			try {
				...
				/* check report class */
				Class rptClass = rf.getReportClass(); // may throw
														// ReportException
				...
				if (!StringTools.isBlank(rptLayout)) {
					...
					try {
						...
						Print.logDebug("Found layout: "
								+ StringTools.className(layoutExpect));
					} catch (Throwable t) {
						Print.logError("[" + xmlFile + "] Report '" + rptName
								+ "' [Specified layout not found]");
					...
					try {
						if (layoutExpect != null) {
							MethodAction ma = new MethodAction(rptClass,
									"GetReportLayout");
						...
						// Report likely does not responds to method
						// "GetReportLayout"
				...
				if (rptOptMap != null) {
					...
					try {
						MethodAction rptOptProvMA = new MethodAction(rptClass,
								"getReportOptionsProvider");
						ReportOptionsProvider rptOptProv = (ReportOptionsProvider) rptOptProvMA
								.invoke();
						if (rptOptProv != null) {
				...
				if (!IGNORE_MISSING_REPORTS && !rptOptional) {
					...
					Print.logWarn("Optional Report '" + rptName + "' ["
							+ re.getMessage() + "]");
					rfv.setHasParsingErrors();
				} else {
		...
		for (int ic = 0; ic < inclList.getLength(); ic++) {
			...
			boolean optional = XMLTools.getAttributeBoolean(include,
					ATTR_optional, false, false);
			String dirStr = XMLTools.getAttribute(include, ATTR_dir, null,
					false);
			File inclDir = !StringTools.isBlank(dirStr) ? new File(dirStr)
					: null;
			String inclFile = XMLTools.getAttribute(include, ATTR_file, null,
					false);
			...
			// 3) <AbsoluteIncludeDir>/<IncludeFile> (absolute dir/file
			// specification)
			...
			if ((foundInclFile != null) && foundInclFile.isFile()) {
				try {
					...
					Print.logError("Error while including file: "
							+ foundInclFile);
					Print.logException("Error while including file: "
							+ foundInclFile, th);
				}
			} else if (!optional) {
	...
	private static ReportConstraints parseReportConstraintsXML(
			ReportFactoryVars rfv, Element dftConst, RTProperties rptDefProps) {
		File xmlFile = rfv.getXMLFile();
		...
		for (int c = 0; c < attrList.getLength(); c++) {
			...
			if (attrName.equalsIgnoreCase(TAG_TimeStart)) {
				rc.setTimeStart(StringTools.parseLong(
						ReportFactory.getNodeText(attrElem, " "), -1L));
				// Print.logInfo("TimeStart: " + rc.getTimeStart());
			} else if (attrName.equalsIgnoreCase(TAG_TimeEnd)) {
				rc.setTimeEnd(StringTools.parseLong(
						ReportFactory.getNodeText(attrElem, " "), -1L));
				// Print.logInfo("TimeEnd: " + rc.getTimeEnd());
			} else if (attrName.equalsIgnoreCase(TAG_ValidGPSRequired)) {
				rc.setValidGPSRequired(StringTools.parseBoolean(
						ReportFactory.getNodeText(attrElem, " "), false));
				// Print.logInfo("ValidGPSRequired: " +
				// rc.getValidGPSRequired());
			} else if (attrName.equalsIgnoreCase(TAG_OrderAscending)) {
				rc.setOrderAscending(StringTools.parseBoolean(
						ReportFactory.getNodeText(attrElem, " "), true));
				// Print.logInfo("OrderAscending: " + rc.getOrderAscending());
			} else if (attrName.equalsIgnoreCase(TAG_OrderDescending)) {
				rc.setOrderAscending(!StringTools.parseBoolean(
						ReportFactory.getNodeText(attrElem, " "), false));
				// Print.logInfo("OrderAscending: " + rc.getOrderAscending());
			} else if (attrName.equalsIgnoreCase(TAG_SelectionLimit)) {
				String limStr = rptDefProps.insertKeyValues(ReportFactory
						.getNodeText(attrElem, " "));
				...
				} else if (typeStr.equalsIgnoreCase("last")) {
					type = EventData.LimitType.LAST;
				} else {
					type = (limit > 0L) ? EventData.LimitType.LAST
							: EventData.LimitType.FIRST;
				}
				rc.setSelectionLimit(type, limit);
				// Print.logInfo("Limit: (type=" + type + ") " + limit);
			} else if (attrName.equalsIgnoreCase(TAG_ReportLimit)) {
				String limStr = rptDefProps.insertKeyValues(ReportFactory
						.getNodeText(attrElem, " "));
				long limit = StringTools.parseLong(limStr, -1L);
				rc.setReportLimit(limit);
			} else if (attrName.equalsIgnoreCase(TAG_Where)) {
				String typeStr = attrElem.getAttribute(ATTR_type); // <--
																	// currently
																	// ignored
				rc.setWhere(ReportFactory.getNodeText(attrElem, " "));
				// Print.logInfo("Where: " + rc.getWhere());
			} else if (attrName.equalsIgnoreCase(TAG_RuleSelector)) {
				String rfName = attrElem.getAttribute(ATTR_ruleFactoryName);
				RuleFactory ruleFact = Device.getRuleFactory(true);
				if (ruleFact == null) {
					// no Device RuleFactory installed
				} else if ((StringTools.isBlank(rfName) || rfName
						.equalsIgnoreCase(ruleFact.getName()))) {
					String ruleSel = ReportFactory.getNodeText(attrElem, " ");
					if (!ruleFact.checkSelectorSyntax(ruleSel)) {
						Print.logWarn("[" + xmlFile
								+ "] Invalid RuleSelector syntax: " + ruleSel);
					} else {
					...
					// Print.logWarn("[" +xmlFile +
					// "] Ignoring RuleSelector for RuleFactory '"+rfName+"'");
				}
			} else {
				Print.logError("[" + xmlFile + "] Unrecognized tag: "
						+ attrName);
				rfv.setHasParsingErrors();
			}
	...
	protected static ReportType _getReportType(String type) {
		return ((ReportTypeMap != null) && (type != null)) ? ReportTypeMap
				.get(type) : null;
	}
	...
	public static void addReportType(String type, boolean isGroup,
			I18N.Text desc) {
		if (!StringTools.isBlank(type)) {
			if (ReportTypeMap == null) {
				ReportTypeMap = new HashMap<String, ReportType>();
			}
	...
	public static ReportFactory getReportFactory(String rptName,
			boolean isOptional) throws ReportException {
	...
	/* parse I18N key/text */
	protected static I18N.Text parseI18N(String pkgName, String i18nKey,
			String dftStr) {
		...
		if (this.classObj == null) {
			...
			try {
				...
				if (!ReportData.class.isAssignableFrom(rptClass)) {
					throw new ReportException(cn
							+ " does not implement interface ReportData");
				}
	...
	public ReportData createReport(ReportEntry reportEntry,
			String reportOptionID, RequestProperties reqState, Device device)
			throws ReportException {
		String rptClassName = StringTools.className(this.getReportClass());
		if (device == null) {
			// Print.logInfo("Creating Group 'All' Report: " + rptClassName);
			return this._createReport(reportEntry, reportOptionID, reqState,
					(ReportDeviceList) null);
		} else {
			// Print.logInfo("Creating Device '"+device+"' Report: " +
			// rptClassName);
			Account account = reqState.getCurrentAccount();
			User user = reqState.getCurrentUser();
			return this._createReport(reportEntry, reportOptionID, reqState,
					new ReportDeviceList(account, user, device));
		}
	}

	public ReportData createReport(ReportEntry reportEntry,
			String reportOptionID, RequestProperties reqState, DeviceGroup group)
			throws ReportException {
		String rptClassName = StringTools.className(this.getReportClass());
		if (group == null) {
			// Print.logInfo("Creating Group 'All' Report: " + rptClassName);
			...
			// Print.logInfo("Creating Group '"+group+"' Report: " +
			// rptClassName);
			Account account = reqState.getCurrentAccount();
			User user = reqState.getCurrentUser();
			return this._createReport(reportEntry, reportOptionID, reqState,
					new ReportDeviceList(account, user, group));
		}
	}

	public ReportData createReport(ReportEntry reportEntry,
			String reportOptionID, RequestProperties reqState,
			ReportDeviceList deviceList) throws ReportException {
	...
	protected ReportData _createReport(ReportEntry reportEntry,
		...
		if ((reportEntry != null) && (reportEntry.getReportFactory() != this)) {
			throw new ReportException("Invalid ReportEntry: "
					+ this.getReportName());
		}
		...
		try {
			if (reportEntry != null) {
				...
				Class argTypes[] = new Class[] { ReportEntry.class,
						RequestProperties.class, ReportDeviceList.class };
				MethodAction rc = new MethodAction(rptClass,
						MethodAction.CONSTRUCTOR, argTypes);
				reportInstance = rc.invoke(new Object[] { reportEntry,
						reqState, deviceList });
			} else {
				Print.logWarn("ReportEntry not specified ...");
				throw new Throwable(
						"No ReportEntry ... try again using ReportFactory");
			}
		} catch (ReportException re) {
			throw re; // re-throw
		} catch (Throwable reTh) { // NoSuchMethodException,
									// ClassNotFoundException
			// try legacy 'ReportFactory' generation
			try {
				Class argTypes[] = new Class[] { ReportFactory.class,
						RequestProperties.class, Account.class, User.class,
						ReportDeviceList.class };
				MethodAction rc = new MethodAction(rptClass,
						MethodAction.CONSTRUCTOR, argTypes);
				Account account = reqState.getCurrentAccount();
				User user = reqState.getCurrentUser();
				reportInstance = rc.invoke(new Object[] { this, reqState,
						account, user, deviceList });
				Print.logInfo("Report not yet converted to new constructor: "
						+ this.getReportName());
			} catch (ReportException re) {
				throw re; // re-throw
			} catch (Throwable reTh2) {
				throw new ReportException("Unable to create report: "
						+ this.getReportClassName(), reTh);
			}
		}
		...
		if (!(reportInstance instanceof ReportData)) {
			throw new ReportException(
					"Report class is not a subclass of ReportData");
		}
		...
		/* report option */
		ReportOption rptOpt = this.getReportOption(reportOptionID, reqState); // may
																				// be
																				// null
	...
	public String getMenuDescription(Locale loc, String dft) {
		return (this.menuDescription != null) ? this.menuDescription
				.toString(loc) : dft;
	}
	...
	public String getReportTitle(Locale loc, String dft) {
		return (this.reportTitle != null) ? this.reportTitle.toString(loc)
				: dft;
	}
	...
	public String getReportSubtitle(Locale loc, String dft) {
		return (this.reportSubtitle != null) ? this.reportSubtitle
				.toString(loc) : dft;
	}

	/* return report subtitle */
	public String getReportSubtitle(Locale loc) {
		return this.getReportSubtitle(loc,
				"${deviceDesc} [${deviceId}]\n${dateRange}");
	}
	...
	public ReportColumn[] getReportColumns() {
		return (this.reportColumns != null) ? this.reportColumns
				: new ReportColumn[0];
	}
	...
	public void setMapIconSelector(String iconSel) {
		this.mapIconSelector = ((iconSel != null) && !iconSel.equals("")) ? iconSel
				: null;
	}
	...
	public OrderedMap<String, ReportOption> getReportOptionMap(
		...
		} else if (this.reportOptionsProvider != null) {
			return this.reportOptionsProvider
					.getReportOptionMap(this, reqState);
		} else {
			return null;
		}
	}
	...
	public ReportOption getReportOption(String name, RequestProperties reqState) {
		if (name == null) {
			...
			OrderedMap<String, ReportOption> rptOpt = this
					.getReportOptionMap(reqState);
	...
	public OrderedMap<String, String> getReportOptionDescriptionMap(
			RequestProperties reqState) {
		OrderedMap<String, ReportOption> rptOptMap = this
				.getReportOptionMap(reqState);
	...
	public static void main(String argv[]) {
		...
		File reportXML = RTConfig.getFile(ARG_XML, new File("./"
				+ REPORT_FACTORY_XML));
		...
		/* report constraints */
		Print.logInfo("Attempting to load Account/Device: " + accountID + "/"
				+ deviceID);
		Account acct = null;
		Device dev = null;
		try {
			...
			if (acct != null) {
				...
				if (dev == null) {
					Print.logError("Device not found: " + accountID + "/"
							+ deviceID);
					System.exit(1);
				}
			} else {
		...
		try {
			...
			if (rptInfo == null) {
				...
				Print.logInfo("ReoprtFactory constraints: "
						+ rf.getReportConstraints());
				Print.logInfo("ReoprtData constraints   : "
						+ rpt.getReportConstraints());
			}
		} catch (ReportException re) {
