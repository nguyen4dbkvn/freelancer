...
// ----------------------------------------------------------------------------
/* minhnv*/
function MinhNVScrollBar(){
	
	$('#MinhNVScrollBar').tinyscrollbar();
	document.getElementById('devChooserView').className='devSelectorDiv_hidden';
	//$('#devChooserView').className ='devSelectorDiv_hidden';
	
}
/* this is executed when the page is loaded */
function trackMapOnLoad()
{
minhnv(0);dongHoXang(0);MinhNVScrollBar();
    /* display calendars */
    if (mapCal_fr) {
...
function trackMapPingDevice(devcmd) 
{
    if (!IS_FLEET && mapDevicePing && DEVICE_PING_URL && (DEVICE_PING_URL != "")) { 
        ...
        try{
		rfr =document.getElementById('Text1').value.replace(" ","/");
		rto =document.getElementById('Text2').value.replace(" ","/");
        }catch(e){}
...
function _timerAutoUpdateMap() 
{
    if (--AutoIntervalCount <= 0) {
        ...
        AutoIntervalCount = 30; // start over
    }
