    ...
    extends DBFieldType
{
    ...
    public static long EncodeFault(RTProperties rtp)
    {
        ...
        if (type.equalsIgnoreCase(NAME_OBDII)) {
            String  dtc    = rtp.getString(PROP_DTC_0,""); // "P0071"
    ...
    public static long EncodeFault_OBDII(String dtc)
    {
        long faultCode = TYPE_OBDII;

        /* trim */
        dtc = StringTools.trim(dtc);
        if (dtc.equals("")) {
            return faultCode;
        }
        ...
        faultCode |= EncodeSystem(dtc.charAt(0));   // [MID_MASK]       0x00FFFFFF00000000

        /* encode manufacturer specific and subsystem */
        int mfgCode = StringTools.parseInt(dtc.substring(1,2),0);
        int spid    = (mfgCode != 0)? 0x8000 : 0;
        int subSys  = StringTools.parseInt(dtc.substring(2,5),0);
        spid |= (subSys & 0xFFF);
        faultCode |= EncodeSPID(spid);              // [SPID_MASK]      0x00000000FFFF0000
