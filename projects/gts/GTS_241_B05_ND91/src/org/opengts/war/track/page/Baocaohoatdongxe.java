...
// WebPage interface
    ...
    public String getMenuDescription(RequestProperties reqState, String parentMenuName)
    {
        ...
        I18N         i18n        = privLabel.getI18N(Baocaohoatdongxe.class);
    ...
    /*
    public int getTotal(String tuNgay,String denNgay, String device,String tenAccount) throws IOException
    {
    	...
    	try
    	{
	    	...
	    	DBCamera objcmr = new DBCamera();
	    	ResultSet rs = objcmr.GetTotalReportDevDetail(device, tuNgay, denNgay, tenAccount);
	    	while(rs.next())
	    	{
	    		tong1++;
	    	}
    ...
    //Không dùng nữa
    /* public String LoadReport(String tuNgay,String denNgay,String bienXe ,String tenAccount,int currentPage,int Size,PrivateLabel privLabel) throws IOException
    {
    	...
    	try
    	{
    		...
    		ResultSet rs = objcmr.GetReportDevDetail(bienXe, tuNgay, denNgay, tenAccount, currentPage, Size);
    		//int k = getTotal(bienXe, tuNgay, denNgay, tenAccount);
    		//if(k>0)
    		//{
    		ResultSet rs1 = objcmr.Device_select(bienXe, tenAccount);
    		int xang=0;
    		int dh =0;
    		while(rs1.next())
    		{
    			xang = rs1.getInt("xang");
    			dh=rs1.getInt("DieuHoa");
    		}
    		strscr =strscr+ "<table  id='myTable' width='100%' class='adminSelectTable_sortable' cellspacing='1' ><thead><tr  align='center'><th class='adminTableHeaderCol_sort'>STT</th><th  class='adminTableHeaderCol_sort'>Xe</th><th  class='adminTableHeaderCol_sort'>Th\u1EDDi \u0111i\u1EC3m</th><th width='110px' class='adminTableHeaderCol_sort'>Tr\u1EA1ng th\u00E1i</th><th  class='adminTableHeaderCol_sort'>T\u1ED1c \u0111\u1ED9<br/>(Km/h)</th><th  class='adminTableHeaderCol_sort'>Cửa</th><th  class='adminTableHeaderCol_sort'>T\u1ECDa \u0111\u1ED9</th><th class='adminTableHeaderCol_sort'>\u0110\u1ECBa ch\u1EC9</th><th class='adminTableHeaderCol_sort'>Quãng đường<br/>(Km)</th>";
    		String cua = "Đóng";
    		String dieuhoa="Tắt";
    		String xang1 = "-";
    		if(xang==1)
    		{
    		 ...
    		 strscr=strscr+"<th class='adminTableHeaderCol_sort'>Nhiên liệu<br/>(Lít)</th>";
    		}
    		if(dh==1)
    		{
    		 strscr=strscr+"<th class='adminTableHeaderCol_sort'>Điều hòa</th>";
    		}
    ...
    /*	while (rs.next())
    		{    			
    			...
    			if(rs.getInt("altitude") > 0)
    				xang1 = String.valueOf(rs.getInt("altitude"));
    			if(rs.getInt(9) == 1) cua = "Đóng";
    			else cua = "Mở";
    			if(rs.getInt("DieuHoa") == 1) dieuhoa = "Tắt";
    			else dieuhoa = "Bật";
    			String css ="";
    			if(num%2==0)
    			...
    			i++;
    			record++;
    			String tt=StatusCode.getDescription(tenAccount,Integer.parseInt(rs.getString(4)), privLabel, null);
    			if(rs.getString("tt")!="") tt=tt+" "+doiGio(rs.getInt("tt"));
    			strscr =strscr+"<tr class ="+css+"><td>"+(((currentPage-1)*Size)+i)+"</td><td>"+rs.getString(1)+"</td><td>"+rs.getString(2)+" "+rs.getString(3)+"</td><td>"+tt+"</td><td>"+rs.getString(7)+"</td><td>"+cua+"</td><td>"+rs.getString(5)+"/"+rs.getString(6)+"</td><td>"+rs.getString(8)+"</td><td>"+rs.getDouble("odometerKM")+"</td>";	
    			if(xang==1)
    			{
    				strscr=strscr+"<td>"+ xang1 +"</td>";
    			}
    			if(dh==1)
    			{
    			 
    			 strscr=strscr+"<td>"+dieuhoa+"</td>";
    			}
    			// strscr=strscr+"<td>"+t+"</td>";
    			strscr=strscr+"</tr>";
    		}
    		//strscr =strscr+"<tr><td></td><td></td><td></td><td></td><td></td><td></td><td>Tổng </td><td>"+sumDistance+"</td><td></td></tr>";
    		strscr =strscr+"</table>";
    ...
    public String report(String tungay,String denngay,String devid,String timezone,String accid,PrivateLabel privLabel) throws IOException
    {
    	...
    	try
    	{
    		...
    		EventData ed[]= objcmr.GetReportDetail(tungay, denngay, devid, timezone, accid, null);
    		record=ed.length;
    		strscr += "<div class='record_trans'><b>Tổng số bản ghi cần truyền dữ liệu: " +ed.length +"</b></div>";
    		String str=accid.equalsIgnoreCase("ctyconghoan")==false?"Điều hòa":"Tín hiệu cẩu";
    		strscr =strscr+ "<table  id='myTable' width='100%' class='adminSelectTable_sortable' cellspacing='1' ><thead><tr  align='center'><th class='adminTableHeaderCol_sort'>STT</th><th  class='adminTableHeaderCol_sort'>Xe</th>" +
    				"<th  class='adminTableHeaderCol_sort'>Th\u1EDDi \u0111i\u1EC3m</th><th width='110px' class='adminTableHeaderCol_sort'>Tr\u1EA1ng th\u00E1i</th><th  class='adminTableHeaderCol_sort'>T\u1ED1c \u0111\u1ED9<br/>(Km/h)</th>" +
    				"<th  class='adminTableHeaderCol_sort'>Cửa</th><th  class='adminTableHeaderCol_sort'>T\u1ECDa \u0111\u1ED9</th><th class='adminTableHeaderCol_sort'>Nhiên liệu<br/>(Lít)</th><th class='adminTableHeaderCol_sort'>"+ str +"</th>";
    		String cua = "Đóng";
    		String dieuhoa="Tắt";
    		String xang = "-";
    		 strscr=strscr+"<th class='adminTableHeaderCol_sort'>Quãng đường<br/>(Km)</th>";
    		 strscr=strscr+"<th class='adminTableHeaderCol_sort'>Địa chỉ</th>";
    		strscr=strscr+"</tr></thead>";
    		for(int j =0;j<ed.length;j++)
    		{
//    			cua="-";dieuhoa="-";
//    			if(ed[j].getCua()==1) cua="Đóng";
//    			else if(ed[j].getCua()==0||ed[j].getCua()==2) cua="Mở";
//    			
//    			if(ed[j].getDieuHoa()==1||ed[j].getDieuHoa()==0) dieuhoa="Tắt";
//    			else if(ed[j].getDieuHoa()==2) dieuhoa="Bật";
    			//Mr viet them doan nay
    			if (ed[j].getCua() == 1)
					cua = "Đóng";
				else//cac truong hop con lai
					cua = "Mở";
				if (ed[j].getDieuHoa() == 1)
					dieuhoa = "Tắt";
				else //cac truong hop con lai
					dieuhoa = "Bật";
				//end
    			if(ed[j].getAltitude() > 0)
    				xang = String.valueOf(round(ed[j].getAltitude(),2));
    			String tt="";//bien tinh toan thoi gian dung
				if(ed[j].getStatusCode()==61718) tt="Đang hoạt động";
				else
				{
    			 tt=StatusCode.getDescription(accid,ed[j].getStatusCode(), privLabel, null);
    			...
    			strscr =strscr+"<tr class ="+css+"><td>"+(j+1)+"</td><td>"+ed[j].getDeviceID()+"</td><td>"+day+"</td><td>"+tt+"</td><td>"+round(ed[j].getSpeedKPH(),2)+"</td><td>"+cua+"</td>" +
    					"<td>"+round(ed[j].getLatitude(),5)+"/"+round(ed[j].getLongitude(),5)+"</td><td>"+xang+"</td><td>"+dieuhoa+"</td>";	
    			
    				strscr=strscr+"<td>"+round(ed[j].getOdometerKM(),2)  +"</td>";
    			
    			 strscr=strscr+"<td>"+ed[j].getAddress()+"</td>";
...
//                response.setHeader("Content-Disposition", "attachment; filename=baoCaoHoatDongCuaXe_"+ d + ".xls");
                response.setHeader("Content-Disposition", "attachment; filename=ChiTietHoatDongCuaXe_"+ d + ".xls");
		        ...
		        cst.setFont(ft);
		     	HSSFCell ct = title.createCell((short) 0);
       			...
       			ct.setCellStyle(cst);
//       			ct.setCellValue("BÁO CÁO HOẠT ĐỘNG CỦA XE");
       			ct.setCellValue("CHI TIẾT HOẠT ĐỘNG CỦA XE");
              	...
              	HSSFCell h4= rowhead.createCell((short) 4);
		       	...
		       	h6.setCellValue("Nhiên liệu (L)");
      	
		       	HSSFCell h7= rowhead.createCell((short) 7);
		       	h7.setCellStyle(cellStyle);	
		       	h7.setCellValue(currAcct.getAccountID().equalsIgnoreCase("ctyconghoan")==false?"Điều hòa":"Tín hiệu cẩu");
  	    	
		       	HSSFCell h8= rowhead.createCell((short) 8);
       	    	h8.setCellStyle(cellStyle);	
       	    	h8.setCellValue("Quãng đường ");
       	    	
	       	 	HSSFCell h9= rowhead.createCell((short) 9);
		       	h9.setCellStyle(cellStyle);	
		       	h9.setCellValue("Địa chỉ");
       	    	
		       	try{
		       		DBCamera objcmr = new  DBCamera();
		       		rowhead.setHeightInPoints((short)40); 
		       		//ResultSet rs = objcmr.GetCamera(ngay, device, page, pagesize);
               		EventData ed[] = objcmr.GetReportDetail(tuNgay, denNgay,xe,currAcct.getTimeZone(), currAcct.getAccountID(), null);
         			...
         			for(int j =0;j<ed.length;j++)
         			{
         				...
         				if(ed[j].getDieuHoa() == 1) dieuhoa = "Tắt";
            			else dieuhoa = "Bật";
        	        	...
        	        	//row.createCell((short) 2).setCellValue(rs.getString(4));
          				...
          				r2.setCellStyle(csr);
          				r2.setCellValue(GetUTF8FromNCRDecimalString(tt));
          				...
          				r6.setCellStyle(csr);
          				r6.setCellValue(ed[j].getAltitude());
          				
          				HSSFCell r7=row.createCell((short) 7);
          				r7.setCellStyle(csr);
          				r7.setCellValue(GetUTF8FromNCRDecimalString(dieuhoa));
          				
          				HSSFCell r8=row.createCell((short) 8);
          				r8.setCellStyle(csr);
          				r8.setCellValue(round(ed[j].getOdometerKM(),2));
          				
          				HSSFCell r9=row.createCell((short) 9);
          				r9.setCellStyle(csr);
          				r9.setCellValue(GetUTF8FromNCRDecimalString(ed[j].getAddress()));
        ...
        HTMLOutput HTML_CSS = new HTMLOutput() {
            public void write(PrintWriter out) throws IOException {
                String cssDir = Baocaohoatdongxe.this.getCssDirectory();
...
//                out.println("<span class='"+CommonServlet.CSS_MENU_TITLE+"'>B\u00E1o c\u00E1o ho\u1EA1t \u0111\u1ED9ng c\u1EE7a xe</span><br/>");
                out.println("<span class='"+CommonServlet.CSS_MENU_TITLE+"'>Chi Tiết Hoạt Động Của Xe</span><br/>");
                out.println("<hr/>");
               ...
               else
               {
                ...
                if(xem.equals("Xem"))
                {
                	...
                	out.print(report(tuNgay,denNgay,contentall,currAcct.getTimeZone(),currAcct.getAccountID(),privLabel));
                // out.print(sql);
             }     
