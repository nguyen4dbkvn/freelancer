...
JSMap.prototype._addPushpin = function(pp)
{
    try {
      ...
      //tanvm: add lable box
		var label = new ELabel(pt, pp.label, "pushpinLabel");
		alert(label);
       	this.gmapGoogleMap.addOverlay(label); 
    } catch(e) {
        //
    }
};
...
function load() {
    if (GBrowserIsCompatible()) {
      var map = new GMap2(document.getElementById("jsmap"));
      map.addControl(new GSmallMapControl());
      map.addControl(new GMapTypeControl());
      var center = new GLatLng(21.01780,105.78228);
