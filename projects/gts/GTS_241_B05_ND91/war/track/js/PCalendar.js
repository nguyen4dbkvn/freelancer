...
switch(languageCode)
{
    case"en":
        ...
        var weekString='Week';var todayString='';
        break;
    case"vi":
        ...
        var monthArrayShort=['Tháng Giêng','Tháng Hai','Tháng Ba','Tháng Tư','Tháng Năm','Tháng Sáu','Tháng Bảy','Tháng Tám','Tháng Chín','Tháng Mười','Tháng Mười Một','Tháng Mười Hai'];
        var dayArray=['Hai','Ba','Tư','Năm','Sáu','Bảy','CN'];
        var weekString='Tuần';
        var todayString='Hôm nay Thứ';break;
}
...
{
if(!iframeObj2)
    return;iframeObj2.style.display='block';
...
var returnDateToYear;var returnDateToMonth;var returnDateToDay;var returnDateToHour;var returnDateToMinute;var inputYear;var inputMonth;var inputDay;var inputHour;var inputMinute;var calendarDisplayTime=false;var selectBoxHighlightColor='#D60808';var selectBoxRolloverBgColor='#E2EBED';var selectBoxMovementInProgress=false;var activeSelectBox=false;
...
function updateHourDiv()
{
    ...
    for(var no=1;no<hourItems.length-1;no++)
    {
        ...
        {
            hourItems[no].style.color=selectBoxHighlightColor;activeSelectBoxHour=hourItems[no];
        }
        else
        {
...
function writeCalendarContent()
{
    ...
    d.setDate(1);d.setMonth(currentMonth);
    ...
    {
        var cell=row.insertCell(-1);cell.innerHTML=dayArray[no];
    }
    var row=calTBody.insertRow(-1);
    if(showWeekNumber)
    {
    ...
    for(var no=1;no<=daysInMonth;no++)
    {
        ...
        if(colCounter>0&&colCounter%7==0)
        {
            ...
            if(showWeekNumber)
            {
                var cell=row.insertCell(-1);cell.className='calendar_week_column';
                var week=getWeek(currentYear,currentMonth,no);cell.innerHTML=week;
            }
        }
        var cell=row.insertCell(-1);
        ...
        //if((currentYear<inputYear) || ((currentYear == inputYear) && (currentMonth < inputMonth)) || ((currentYear == inputYear) && (currentMonth == inputMonth) && (no<inputDay)))
        //{
...
function displayCalendar(inputField,format,buttonObj,displayTime,timeInput,textBox)
{
    textboxId=textBox;
...
function displayCalendarSelectBox(yearInput,monthInput,dayInput,hourInput,minuteInput,buttonObj)
{
    ...
    if(hourInput)
    {
        currentHour=hourInput.options[hourInput.selectedIndex].value;inputHour=currentHour/1;
    }
    if(minuteInput)
    {
        currentMinute=minuteInput.options[minuteInput.selectedIndex].value;inputMinute=currentMinute/1;
    }
...
function pickDate(e,inputDay)
{
    ...
    if(returnFormat)
    {
		...
		returnFormat=returnFormat.replace('hh',"0"+currentHour);
		else
    ...
    else
    {
        for(var no=0;no<returnDateToYear.options.length;no++)
        {
            ...
            {
                returnDateToYear.selectedIndex=no;break;
            }
        }
        for(var no=0;no<returnDateToMonth.options.length;no++)
        {
            ...
            {
                returnDateToMonth.selectedIndex=no;break;
            }
        }
        for(var no=0;no<returnDateToDay.options.length;no++)
        {
            ...
            {
                returnDateToDay.selectedIndex=no;break;
            }
        }
        if(calendarDisplayTime)
        {
            for(var no=0;no<returnDateToHour.options.length;no++)
            {
                ...
                {
                    returnDateToHour.selectedIndex=no;break;
                }
            }
            for(var no=0;no<returnDateToMinute.options.length;no++)
            {
                ...
                {
                    returnDateToMinute.selectedIndex=no;break;
                }
            }
        }
    }
    closeCalendar();
    var zone=document.getElementById('hidZoneID');
    var parentZone=document.getElementById('hidParentZoneID');
    document.getElementById(textboxId).value = returnFormat;
    //location.href="/"+parentZone.value+"d"+zone.value+"/"+returnFormat+".htm";
}
function getWeek(year,month,day)
{
    ...
    var d1=((d4-L)%365)+L;NumberOfWeek=Math.floor(d1/7)+1;
    return NumberOfWeek;
}
...
function initCalendar()
{if(MSIE){iframeObj=document.createElement('IFRAME');iframeObj.style.filter='alpha(opacity=0)';iframeObj.style.position='absolute';iframeObj.border='0px';iframeObj.style.border='0px';iframeObj.style.backgroundColor='#FF0000';iframeObj2=document.createElement('IFRAME');iframeObj2.style.position='absolute';iframeObj2.border='0px';iframeObj2.style.border='0px';iframeObj2.style.height='1px';iframeObj2.style.width='1px';iframeObj2.src='#';iframeObj.src='#';document.body.appendChild(iframeObj2);document.body.appendChild(iframeObj);}
...
{if(!calendarDisplayTime){document.getElementById('timeBar').style.display='none';document.getElementById('timeBar').style.visibility='hidden';document.getElementById('todaysDateString').style.width='100%';}else{document.getElementById('timeBar').style.display='block';document.getElementById('timeBar').style.visibility='visible';document.getElementById('hourDropDown').style.top=document.getElementById('calendar_minute_txt').parentNode.offsetHeight+calendarContentDiv.offsetHeight+document.getElementById('topBar').offsetHeight+'px';document.getElementById('minuteDropDown').style.top=document.getElementById('calendar_minute_txt').parentNode.offsetHeight+calendarContentDiv.offsetHeight+document.getElementById('topBar').offsetHeight+'px';document.getElementById('minuteDropDown').style.right='50px';document.getElementById('hourDropDown').style.right='50px';document.getElementById('todaysDateString').style.width='180px';document.getElementById('todaysDateString').style.fontSize='9px';}}
