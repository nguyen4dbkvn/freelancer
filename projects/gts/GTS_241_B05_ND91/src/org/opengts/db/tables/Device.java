    ...
    implements DataTransport
{
    ...
    /* simulate engine hours from ignition state */
    ...
    public  static final boolean CHECK_LAST_ODOMETER        = true;
  ...
  //public static final String FLD_smsGatewayProps       = "smsGatewayProps";       // SMS gateway properties
    ...
    public static final String FLD_lastIgnitionOnTime    = "lastIgnitionOnTime";    // last ignition-on time ('0' if ignition is off)
    public static final String FLD_lastIgnitionOffTime   = "lastIgnitionOffTime";   // last ignition-off time ('0' if ignition is on)
    //
        ...
        extends DeviceKey<Device>
    {
        ...
        public Key(String acctId, String devId) {
           ...
           // super.setFieldValue(FLD_deviceID , ((devId  != null)? devId.toLowerCase()  : ""));
            //Minhnv bo in chu thuong
            super.setFieldValue(FLD_deviceID , ((devId  != null)? devId : ""));
        }
    ...
    **/
    public double adjustOdometerKM(double odomKM)
    {
        ...
        if (Device.GetCheckLastOdometer() && (odomKM < lastOdomKM)) {
            return lastOdomKM;
        } else
        if (odomKM >= this.getMaxOdometerKM()) {
            return lastOdomKM;
    ...
    **/
    protected boolean _insertEventData(final EventData evdb)
    {
        ...
        /* clear lastIgnOn/lastIgnOff if necessary */
        if (lastIgnOn < lastIgnOff) {
            lastIgnOn  = 0L;
        } else
        if (lastIgnOff < lastIgnOn) {
        ...
        /* simulated engine-hours (EXPERIMENTAL) */
        if (SIMULATE_ENGINE_HOURS && (lastIgnOn > 0L) && (eventTime > lastIgnOn)) {
            ...
            if (evdb.getEngineHours() > 0.0) {
                ...
                double engHrs = this.getLastEngineHours() + runHrs;
                evdb.setEngineHours(engHrs);
            }
        }
        ...
        if (ignStateCh == 1) { // Ignition is now ON
            // ignition has change from OFF to ON
            if (lastIgnOn > 0L) {
                // last ignition on already set (leave as-is)
                Print.logWarn("Ignition-On event found, without interleaving Ignition-Off");
            } else {
        ...
        if (ignStateCh == 0) { // Ignition is now OFF
            // ignition has change from ON to OFF
            if (lastIgnOff > 0L) {
                // last ignition off already set (leave as-is)
                Print.logWarn("Ignition-Off event found, without interleaving Ignition-On");
            } else {
    ...
    **/
        ...
        throws DBException
    {
        ...
        /* get data */
    	//System.out.println("start getRangeEvents");
    ...
    **/
    ...
    public static String getLastImageLink(String devID){
    	String link="";
    	
    	try {
    		DBCamera objcmr = new DBCamera();
    		ResultSet rs = objcmr.getImgLink(devID);
    		while(rs.next())
    		{
    			link= rs.getString(1);
    ...
    /*Minhnv get last fuel for device - BEGIN */
    public static String getLastFuel(String devID,String acc)
    {
    	String fuel ="";
    	
    		DBCamera objcmr = new DBCamera();
    		fuel = objcmr.getLastFuel(devID,acc);
    		
    	return fuel;
    	
    }
    /*Minhnv get last fuel for device - END */
    ...
    **/
        ...
        throws DBException
    {
        ...
        try {
            ...
            DBWhere dwh = dsel.createDBWhere();
            if (inclInactv) {
                dsel.setWhere(dwh.WHERE_(dwh.AND(  
                		dwh.EQ(Device.FLD_accountID,acctId),
                        dwh.NE(Device.FLD_isBlock,0)
                        )));
            } else {
    ...
    private static final String KEY_DRIVER[]        = EventData.KEY_DRIVER;
    ...
    public String getFieldValueString(String key, String arg, BasicPrivateLabel bpl)
    {
        ...
        if (EventData._keyMatch(key,Device.KEY_DRIVERID)) {
            ...
            String d[] = this.getAttachedEntityIDs(EntityManager.EntityType.DRIVER);
        ...
        if (EventData._keyMatch(key,Device.KEY_DRIVER)) {
            // first check DriverID
            String driverID = this.getDriverID();
            if (!StringTools.isBlank(driverID)) {
                try {
            ...
            // next try attached Driver Entities
            String d[] = this.getAttachedEntityDescriptions(EntityManager.EntityType.DRIVER);
            if (!ListTools.isEmpty(d)) {
                return StringTools.join(d,",");
            }
            // return blank
        ...
        if (EventData._keyMatch(key,Device.KEY_CORRIDORID)) {
            String actvCorr = this.getActiveCorridor();
            return actvCorr;
        }
