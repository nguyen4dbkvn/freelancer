...
public class AccountInfo extends WebPageAdaptor implements Constants {
	...
	public static final String PARM_TAX = "a_tax";
	...
	public String getMenuDescription(RequestProperties reqState,
		...
		return super._getMenuDescription(reqState, i18n.getString(
				"AccountInfo.editMenuDesc", "View/Edit Account Information"));
	}
	...
	public String getMenuHelp(RequestProperties reqState, String parentMenuName) {
		...
		return super._getMenuHelp(reqState, i18n.getString(
				"AccountInfo.editMenuHelp",
				"View and Edit the current Account information"));
	}
	...
	public String getNavigationDescription(RequestProperties reqState) {
		...
		return super._getNavigationDescription(reqState,
				i18n.getString("AccountInfo.navDesc", "Account"));
	}
	...
	public void writePage(final RequestProperties reqState, String pageMsg)
		...
		/* ACL allow edit/view */
		boolean allowEdit = privLabel.hasWriteAccess(currUser,
				this.getAclName());
		boolean allowView = allowEdit
				|| privLabel.hasReadAccess(currUser, this.getAclName());
		...
		if (updateAccount) {
			...
			String submit = AttributeTools.getRequestString(request,
					PARM_SUBMIT_CHANGE, "");
			if (SubmitMatch(submit,
					i18n.getString("AccountInfo.change", "Change"))) {
				String acctName = AttributeTools.getRequestString(request,
						PARM_ACCT_DESC, "");
				String contactName = AttributeTools.getRequestString(request,
						PARM_CONTACT_NAME, "");
				String tax = AttributeTools.getRequestString(request,
						PARM_TAX, "");			
				String contactPhone = AttributeTools.getRequestString(request,
						PARM_CONTACT_PHONE, "");
				String contactEmail = AttributeTools.getRequestString(request,
						PARM_CONTACT_EMAIL, "");
				String notifyEmail = AttributeTools.getRequestString(request,
						PARM_NOTIFY_EMAIL, "");
				String timeZone = AttributeTools.getRequestString(request,
						PARM_TIMEZONE, "");
				String speedUnits = AttributeTools.getRequestString(request,
						PARM_SPEED_UNITS, "");
				String distUnits = AttributeTools.getRequestString(request,
						PARM_DIST_UNITS, "");
				String volUnits = AttributeTools.getRequestString(request,
						PARM_VOLM_UNITS, "");
				String econUnits = AttributeTools.getRequestString(request,
						PARM_ECON_UNITS, "");
				String pressUnits = AttributeTools.getRequestString(request,
						PARM_PRESS_UNITS, "");
				String tempUnits = AttributeTools.getRequestString(request,
						PARM_TEMP_UNITS, "");
				String latLonFormat = AttributeTools.getRequestString(request,
						PARM_LATLON_FORMAT, "");
				String deviceTitle = AttributeTools.getRequestString(request,
						PARM_DEVICE_TITLE, "");
				String devicesTitle = AttributeTools.getRequestString(request,
						PARM_DEVICES_TITLE, "");
				String groupTitle = AttributeTools.getRequestString(request,
						PARM_GROUP_TITLE, "");
				String groupsTitle = AttributeTools.getRequestString(request,
						PARM_GROUPS_TITLE, "");
				String addrTitle = AttributeTools.getRequestString(request,
						PARM_ADDRESS_TITLE, "");
				String addrsTitle = AttributeTools.getRequestString(request,
						PARM_ADDRESSES_TITLE, "");
				String defaultUser = AttributeTools.getRequestString(request,
						PARM_DEFAULT_USER, "");
				try {
					boolean saveOK = true;
					// description
					...
					if (StringTools.isBlank(contactEmail)
						...
						Print.logWarn("Contact EMail address is invalid: "
								+ contactEmail);
						m = i18n.getString(
								"AccountInfo.pleaseEnterContactEMail",
					...
					if (StringTools.isBlank(notifyEmail)) {
						if (!currAcct.getNotifyEmail().equals(notifyEmail)) {
							...
							.validateAddresses(notifyEmail, true/* acceptSMS */)) {
						if (!currAcct.getNotifyEmail().equals(notifyEmail)) {
							currAcct.setNotifyEmail(notifyEmail);
						}
					} else {
						m = i18n.getString(
								...
								"Please enter a valid notify email/sms address");
						error = true;
						saveOK = false;
					}
					// timezone
					...
					if (saveOK) {
						/*
						 * exclude fields that only the SysAdmin/AccountManager
						 * should change
						 */
						currAcct.addExcludedUpdateFields(Account.FLD_isActive,
						...
						AttributeTools.setSessionAttribute(request,
								Calendar.PARM_TIMEZONE[0], timeZone);
						// Track.writeMessageResponse(reqState,
						// i18n.getString("AccountInfo.updatedAcct","Account information updated"));
						m = i18n.getString("AccountInfo.updatedAcct",
								"Account information updated");
					}
				} catch (Throwable t) {
					Print.logException("Updating Account", t);
					m = i18n.getString("AccountInfo.errorUpdate",
							"Internal error updating Account");
					error = true;
					return;
				}
			}
		...
		HTMLOutput HTML_CSS = new HTMLOutput() {
			public void write(PrintWriter out) throws IOException {
				...
				// WebPageAdaptor.writeCssLink(out, reqState, "AccountInfo.css",
				// cssDir);
				// out.println("<link type=\"text/css\" href=\"./css/controls.css\" rel=\"Stylesheet\">");
				// out.println("<link type=\"text/css\" href=\"./custom/styleCombobox.css\" rel=\"Stylesheet\">");
			}
		};
		...
		final ComboMap _suList = privLabel
				.getEnumComboMap(Account.SpeedUnits.class);
		final ComboMap _duList = privLabel
				.getEnumComboMap(Account.DistanceUnits.class);
		final ComboMap _vuList = privLabel
				.getEnumComboMap(Account.VolumeUnits.class);
		final ComboMap _ecList = privLabel
				.getEnumComboMap(Account.EconomyUnits.class);
		final ComboMap _puList = privLabel
				.getEnumComboMap(Account.PressureUnits.class);
		final ComboMap _tuList = privLabel
				.getEnumComboMap(Account.TemperatureUnits.class);
		final ComboMap _llList = privLabel
				.getEnumComboMap(Account.LatLonFormat.class);
		// final ComboMap _rgList =
		// privLabel.getEnumComboMap(Account.GeocoderMode.class );
		...
		HTMLOutput HTML_CONTENT = new HTMLOutput(
			...
			public void write(PrintWriter out) throws IOException {
				...
				String frameTitle = _allowEdit ? i18n.getString(
						"AccountInfo.editAccount", "Edit Account Information")
						: i18n.getString("AccountInfo.viewAccount",
				...
				// ComboOption geocoderMode =
				// privLabel.getEnumComboOption(Account.getGeocoderMode(currAcct)
				// );
				...
				// out.println(FormRow_TextField(PARM_ACCT_ID
				// ,i18n.getString("AccountInfo.accountID","Account ID:") ,
				// false , currAcct.getAccountID(),"" , 40, 32,"inputText")); //
				// read-only
				out.println(FormRow_TextField(PARM_ACCT_ID, false,
						i18n.getString("AccountInfo.accountID", "Account ID:"),
						currAcct.getAccountID(), 40, 32)); // read-only
				...
				out.println(FormRow_TextField(PARM_TAX, _allowEdit, "Mã số thuế: ", currAcct.getTax(), 40, 40));
				out.println(FormRow_TextField(PARM_CONTACT_NAME, _allowEdit,
						i18n.getString("AccountInfo.contactName",
								"Contact Name:"), currAcct.getContactName(),
						40, 40));
				out.println(FormRow_TextField(PARM_CONTACT_PHONE, _allowEdit,
						i18n.getString("AccountInfo.contactPhone",
								"Contact Phone:"), currAcct.getContactPhone(),
						40, 20));
				out.println(FormRow_TextField(PARM_CONTACT_EMAIL, _allowEdit,
						i18n.getString("AccountInfo.contactEMail",
								"Contact Email:"), currAcct.getContactEmail(),
						40, 100));
				out.println(FormRow_TextField(PARM_NOTIFY_EMAIL, _allowEdit,
						i18n.getString("AccountInfo.notifyEMail",
								"Notify Email:"), currAcct.getNotifyEmail(),
						40, 125));

				out.println(FormRow_ComboBox(PARM_TIMEZONE, _allowEdit,
						i18n.getString("AccountInfo.timeZone", "Time Zone:"),
						currAcct.getTimeZone(), _tzList, null, 40));

				out.println(FormRow_ComboBox(PARM_SPEED_UNITS, _allowEdit, i18n
						.getString("AccountInfo.speedUnits", "Speed Units:"),
						speedUnits, _suList, null, 40));

				out.println(FormRow_ComboBox(PARM_DIST_UNITS, _allowEdit, i18n
						.getString("AccountInfo.distanceUnits",
								"Distance Units:"), distanceUnits, _duList,
						null, 40));
				out.println(FormRow_ComboBox(PARM_VOLM_UNITS, _allowEdit, i18n
						.getString("AccountInfo.volumeUnits", "Volume Units:"),
						volumeUnits, _vuList, null, 40));
				out.println(FormRow_ComboBox(PARM_ECON_UNITS, _allowEdit,
						i18n.getString("AccountInfo.economyUnits",
								"Economy Units:"), economyUnits, _ecList, null,
						40));
				out.println(FormRow_ComboBox(PARM_PRESS_UNITS, _allowEdit, i18n
						.getString("AccountInfo.pressureUnits",
								"Pressure Units:"), pressureUnits, _puList,
						null, 40));
				out.println(FormRow_ComboBox(PARM_TEMP_UNITS, _allowEdit, i18n
						.getString("AccountInfo.temperatureUnits",
								"Temperature Units:"), temperatureUnits,
						_tuList, null, 40));
				out.println(FormRow_ComboBox(PARM_LATLON_FORMAT, _allowEdit,
						i18n.getString("AccountInfo.latLonFormat",
								"Latitude/Longitude Format:"), latLonFormat,
						_llList, null, 40));

				 
				/* default user */
				out.println(FormRow_TextField(PARM_DEFAULT_USER, _allowEdit,
						i18n.getString("AccountInfo.defaultUser",
								"Default Login UserID:"), currAcct
								.getDefaultUser(), 40, 32));

				/* expiration */
				long expireTime = currAcct.getExpirationTime();
				if (expireTime > 0L) {
					String expireTimeStr = reqState.formatDateTime(expireTime);
					if (StringTools.isBlank(expireTimeStr)) {
						expireTimeStr = "n/a";
					}
					out.println(FormRow_TextField(PARM_ACCT_EXPIRE, false,
							i18n.getString("AccountInfo.expiration",
									"Expiration:"), expireTimeStr, 40, 30)); // read-only
				}
				...
				if (maxPingCnt > 0) {
					...
					int remaining = (maxPingCnt > totPingCnt) ? (maxPingCnt - totPingCnt)
							: 0;
					out.print("<tr>");
					out.print("<td class='"
							+ CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER
							+ "'>"
							+ i18n.getString("AccountInfo.maxCommandCount",
									"Max Allowed Commands") + ":</td>");
					out.print("<td class='"
							+ CommonServlet.CSS_ADMIN_VIEW_TABLE_DATA + "'>");
					out.print(Form_TextField(PARM_MAX_PINGS, false,
							String.valueOf(maxPingCnt), 5, 5));
					out.print("<span style='margin-left: 10px;margin-right:5px;'>"
							+ i18n.getString("AccountInfo.remainingCommands",
									"Remaining Commands") + ":</span>");
					out.print(Form_TextField(PARM_TOT_PINGS, false,
							String.valueOf(remaining), 5, 5));
					out.print("</td>");
					out.println("</tr>");
				}
				...
				if (_allowEdit) {
					out.write("<input type='submit' name='"
							+ PARM_SUBMIT_CHANGE + "' value='"
							+ i18n.getString("AccountInfo.change", "Change")
							+ "' class='button'>\n");
					out.write("<span style='padding-left:10px'>&nbsp;</span>\n");
					out.write("<input type='button' name='"
							+ PARM_BUTTON_CANCEL + "' value='"
							+ i18n.getString("AccountInfo.cancel", "Cancel")
							+ "' onclick=\"javascript:openURL('" + menuURL
							+ "','_self');\" class='button'>\n"); // target='_top'
				} else {
					out.write("<input type='button' name='" + PARM_BUTTON_BACK
							+ "' value='"
							+ i18n.getString("AccountInfo.back", "Back")
							+ "' onclick=\"javascript:openURL('" + menuURL
							+ "','_self');\" class='button'>\n"); // target='_top'
				}
				out.write("</form>\n");
