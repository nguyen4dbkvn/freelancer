...
var JSON_LastEvent = "LastEvent";  // last event time for current
// device
...
var JSON_cmd = "cmd";        // action to perform
// ("autoupdate", "alert",
// "gotourl", etc)
...
var TAG_Action = "Action";     // action to perform
// ("autoupdate", "alert",
// "gotourl", etc)
var TAG_Time = "Time";       // update time (server time)
var TAG_LastEvent = "LastEvent";  // last event time for current
// device
...
var arrLinkImage = new Array();
var arrDeviceID = new Array();
var iLengOfArrayImage = 0;
var countdev=0;
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
// --- JSMapPoint
if(typeof(DD_belatedPNG) != 'undefined'){
	DD_belatedPNG.fix('.speedo');
}
/**
...
//hienntd add set icon for statuscode=61718
function jsmGetPushPinIconForStatusCode(e) {
	
			pp = jsvPushpinIcon[0];
			pp.key="last";
			pp.bgURL="extra/images/pp/CrosshairGreen.gif";
			pp.bgOffset=[16,16];
			pp.iconURL="images/pp/pin30_Status_dot.png";
...
function JSDetailPoint(rcdNdx, dsNdx, ppNdx, evRcd, textColor) {
    ...
    // no
    // pushpin]
    this.index = rcdNdx;                                             // XX
    // [1+]
    this.code = evRcd.code;                                         // A...A
    this.dateTime = evRcd.dateFmt + ' ' + evRcd.timeFmt;                // YYYY/MM/DD
    // HH:MM:SS
	...
	this.deviceId = evRcd.deviceId;
	this.statusLastingTime = evRcd.statusLastingTime
	this.odomKM= evRcd.odomKM;
    /* Minhnv get last image for device - BEGIN */
   // this.lastImageLink = evRcd.lastImageLink;
	this.LastFuel =evRcd.LastFuel;
    /* Minhnv get last image for device - END */
};
    ...
    if (jsmap == null) {
        ...
        if (jsmapElem != null) {
            try {
                ...
                if (jsmap) {
                    jsmap.JSClearLayers();
                    jsmap.JSDrawPushpins(null, RECENTER_ZOOM, 0);  // TODO:
                    // check
                    // "jsvGeozoneMode"?
                } else {
                    // seems to be necessary on IE (it doesn't throw an
                    // exception)
                    alert(
...
function jsmParseAJAXPoints_JSON(jsonText, recenterMode, replay) // tmz
{
    ...
    if (Time != null) {
        ...
        jsvTodayYMD = Time.YMD;               // ATTR_ymd (in selected
        // timezone)
    ...
    if (LastEvent != null) {
        ...
        jsvLastEventYMD = LastEvent.YMD;          // ATTR_ymd (in selected
        // timezone)
        jsvLastEventDateFmt = LastEvent.date;         // ATTR_date
        jsvLastEventTimeFmt = LastEvent.time;         // ATTR_time
        jsvLastBatteryLevel = LastEvent.battery;      // ATTR_battery //
        // battery level (%)
        jsvLastSignalStrength = LastEvent.signal;       // ATTR_signal // signal
        // strength (%)
    } else {
    ...
    if ((Shapes != null) && (Shapes.length > 0)) {
        for (var msi = 0; msi < Shapes.length; msi++) {
            var ms = Shapes[msi];
            var type = ms.type;      // ATTR_type ("circle", "rectangle",
            // "polygon")
            ...
            // alert("Parsed JSON shape type="+type +" radius="+radiusM +"
            // color="+color +" pt="+dbgPts);
    ...
    for (var dsi = 0; dsi < DataSets.length; dsi++) {
        ...
        /* type */
        var type = strDefault(ds.type, "device"); // ATTR_type ("group",
        // "device", "poi")
        ...
        for (var p = startNdx; p < pts.length; p++) {
            ...
            if (isPOI) {
                if (evRcd.validGPS) {
                    ...
                    var ppObj = jsmCreatePushPin(ndx, -1, ppNdx, evRcd);
            ...
            if (!IS_FLEET && (evRcd.timestamp > jsvLastEventEpoch)) {
                ...
                jsvLastEventYMD = { YYYY: evRcd.year, MM: evRcd.month1, DD: evRcd.day }; // in
                // selected
                // timezone
    ...
    /*
    * device breaks? (more than one dataset and any single dataset has more
    * than one point)
    */
    ...
    if (shapes && (shapes.length > 0)) {
        for (var i = 0; i < shapes.length; i++) {
            ...
            // alert("Drew JSON shape type="+s.type + "rad="+s.radius +"
            // ok="+ok);
        }
    }
    ...
    for (var ai = 0; ai < Actions.length; ai++) {
        var act = Actions[ai];
        var cmd = strDefault(act.cmd, "");  // JSON_cmd ("autoupdate", "alert",
        // "gotourl", "zoompp", "showpp")
...
function jsmParseAJAXPoints_XML(xmlText, recenterMode, replay) // tmz
{
    ...
    if (latestVal != null) {
        ...
        var year = getXMLNodeAttribute(timeAttr, ATTR_year, 0);     // year
        // (in
        // selected
        // timezone)
        var month1 = getXMLNodeAttribute(timeAttr, ATTR_month, 0);    // month1
        // (in
        // selected
        // timezone)
        var day = getXMLNodeAttribute(timeAttr, ATTR_day, 0);      // date
        // (in
        // selected
        // timezone)
        var battery = getXMLNodeAttribute(timeAttr, ATTR_battery, 0);  // battery
        // level
        // (%)
        var signal = getXMLNodeAttribute(timeAttr, ATTR_signal, 0);   // signal
        // strength
        // (%)
        jsvLastEventYMD = { DD: day , MM: month1, YYYY: year};              // in
        // selected
        // timezone
        ...
        if (fld.length > 0) {
            var dateFmt = (fld.length > 0) ? fld[0] : '';            // formatted
            // date
            var timeFmt = (fld.length > 1) ? fld[1] : '';            // formatted
            // time
            var battFmt = (fld.length > 2) ? fld[2] : '';            // formatted
            // battery
            // level
            var signFmt = (fld.length > 3) ? fld[3] : '';            // formatted
            // signal
            // strength
    ...
    if (todayVal != null) {
        ...
        // selected
        // timezone)
        jsvTodayYMD = { DD: day, MM: month1, YYYY: year };              // in
        // selected
        // timezone
    ...
    for (var msi = 0; msi < mapShapes.length; msi++) {
        ...
        var type = getXMLNodeAttribute(msAttr, ATTR_type, "none"); // "circle",
        // "rectangle",
        // "polygon"
        var radiusM = numParseFloat(getXMLNodeAttribute(msAttr, ATTR_radius, 0), 0); // fix
        // 2.3.9-B17
        ...
        // alert("Parsed XML shape type="+type +" radius="+radiusM +"
        // color="+color +" pt="+dbgPts);
    ...
    for (var dsi = 0; dsi < dataSets.length; dsi++) {
        ...
        /* type */
        var type = getXMLNodeAttribute(dsAttr, ATTR_type, "device"); // "group",
        // "device",
        // "poi"
        var isPOI = (type == "poi") ? true : false;
        ...
        // alert("Route = "+route+"/"+showRoute+" [" +
        // getXMLNodeAttribute(dsAttr,ATTR_route,"?"));
        ...
        for (var p = startNdx; p < pts.length; p++) {
            ...
            /* Minhnv get last image for device - BEGIN */
			if(evRcd.lastImageLink!=""&&evRcd.device!="")
           { arrLinkImage[iLengOfArrayImage] = evRcd.lastImageLink;
           arrLinkImage[iLengOfArrayImage] = evRcd.LastFuel;
            arrDeviceID[iLengOfArrayImage] = evRcd.device;
            iLengOfArrayImage++;}
            /* Minhnv get last image for device - END */
            // alert(arrDeviceID);
            /* add type/typeID */
            evRcd.type = type;
            evRcd.typeID = typeID;
            //hienntd add show icon have statuscode=61718
         //  if(evRcd.code=="Dừng, đang hoạt động")
            //	evRcd.iconNdx=-1;
            /* Point Of Interest? */
            if (isPOI) {
                if (evRcd.validGPS) {
    ...
    */
    ...
    if (shapes && (shapes.length > 0)) {
        for (var i = 0; i < shapes.length; i++) {
            var s = shapes[i];
            var ok = jsmDrawShape(s.type, s.radius, s.points, s.color, s.zoomTo);
            // alert("Drew XML shape type="+s.type + "rad="+s.radius +"
            // ok="+ok);
        }
    }
    ...
    /* update last event times */
    if (jsvLastEventDateFmt && jsvLastEventTimeFmt) {
        // alert("Updating Latest Event: " + jsvLastEventDateFmt + " " +
        // jsvLastEventTimeFmt + " " + jsvLastEventTmzFmt);
        jsmSetIDInnerHTML(ID_LATEST_EVENT_DATE, jsvLastEventDateFmt);
        jsmSetIDInnerHTML(ID_LATEST_EVENT_TIME, jsvLastEventTimeFmt);
        jsmSetIDInnerHTML(ID_LATEST_EVENT_TMZ, jsvLastEventTmzFmt);
    } else {
    ...
    for (var ai = 0; ai < actions.length; ai++) {
        ...
        var cmd = getXMLNodeAttribute(attr, ATTR_command, ""); // "autoupdate",
        // "alert",
        // "gotourl"
...
/**
* ** Load and display point from the specified URL and display them on the
* current map
*/
...
/**
* ** Call-back on load state change ** -1 == finish error ** 0 == finish
* success ** 1 == start
*/
...
* 
* @param type
*            The Geozone shape type **
* @param radiusM
*            The circle radius, in meters **
* @param points
*            An array of points (JSMapPoint[]) **
* @param zoomTo
*            True to center/zoom to shape **
* @return True if shape was drawn, false otherwise
*/
// minhnv
var url = ["http://img.ngoisao.vn/news/2012/2/23/40/1jpg1329990079.jpg", "http://img.ngoisao.vn/news/2012/2/23/40/2jpg1329990079.jpg",
          "http://img.ngoisao.vn/news/2012/2/23/40/4jpg1329990079.jpg", "http://img.ngoisao.vn/news/2012/2/23/40/14jpg1329990080.jpg"];
var dem = 0;
function next()
{
	if(arrLinkImage[dem]!="")
    {document.getElementById('Minhnvbasic-modal-content').innerHTML ="<img src ='http://dinhvivietnam.vn:8080/ImgGTS/"+arrLinkImage[dem]+"' width='590' height='350' alt='Kh&#x00F4;ng c&#x00F3; &#x1EA3;nh' style='border:1px solid white'/><div style='text-align:center; width:100%;font-weight:bold'>Bi&#x1EC3;n xe:" +arrDeviceID[dem]+
    '</div><div style="position: absolute; width:30px; height:30px;  top:170px; right:40px;"><img src="images/next.png" onclick="next();" width="40px"/></div>'+
    '<div style="position: absolute; width:30px; height:30px;  top:170px; left:40px;"><img src="images/prev.png" onclick="prev();" width="40px"/></div>';
	}
	else
	{document.getElementById('Minhnvbasic-modal-content').innerHTML ="<img src ='http://www.saeds.ca/images/noimage.png' width='590' height='350' alt='Kh&#x00F4;ng c&#x00F3; &#x1EA3;nh' style='border:1px solid white'/><div style='text-align:center; width:100%;font-weight:bold'>Biá»ƒn xe:" +arrDeviceID[dem]+
    '</div><div style="position: absolute; width:30px; height:30px;  top:170px; right:40px;"><img src="images/next.png" onclick="next();" width="40px"/></div>'+
    '<div style="position: absolute; width:30px; height:30px;  top:170px; left:40px;"><img src="images/prev.png" onclick="prev();" width="40px"/></div>';
	}
    $('#Minhnvbasic-modal-content').modal();
          
	if(dem==arrLinkImage.length-1) dem=-1;
	dem++;
}
...
{          
	...
	if(dem==0) dem=arrLinkImage.length;
	dem--;
}
function getContent(content)
    { 
	if (content!="")
   { document.getElementById('Minhnvbasic-modal-content').innerHTML ="<img src ='http://dinhvivietnam.vn:8080/ImgGTS/"+content+"' width='590' height='350' alt='Kh&#x00F4;ng c&#x00F3; &#x1EA3;nh' style='border:1px solid white'/>" +
    '<div style="position: absolute; width:30px; height:30px;  top:170px; right:40px;"><img src="images/next.png" onclick="next();" width="40px"/></div>'+
    '<div style="position: absolute; width:30px; height:30px;  top:170px; left:40px;"><img src="images/prev.png" onclick="prev();" width="40px"/></div>';
	
	}else
	{document.getElementById('Minhnvbasic-modal-content').innerHTML ="<img src ='http://www.saeds.ca/images/noimage.png' width='590' height='350' alt='Kh&#x00F4;ng c&#x00F3; &#x1EA3;nh' style='border:1px solid white'/>"+
    '<div style="position: absolute; width:30px; height:30px;  top:170px; right:40px;"><img src="images/next.png" onclick="next();" width="40px"/></div>'+
    '<div style="position: absolute; width:30px; height:30px;  top:170px; left:40px;"><img src="images/prev.png" onclick="prev();" width="40px"/></div>';
...
{
var bien="";
var bien=bien+"<div id='chiTietHanhTrinh'><div class='scrollbar'><div class='track'><div class='thumb'><div class='end'></div></div></div></div><div class='viewport'><div class='overview'>"+
"<table id='myTable' width='100%'class='adminSelectTable_sortable1' cellspacing='1'><thead><tr align='center'><th class='adminTableHeaderCol_sort'>STT</td><th class='adminTableHeaderCol_sort'>Th&#x1EDD;i &#x0111;i&#x1EC3;m</th><th class='adminTableHeaderCol_sort'>Tr&#x1EA1;ng th&#x00E1;i</th><th class='adminTableHeaderCol_sort'>Tọa độ</th><th class='adminTableHeaderCol_sort'>Km/h</th><th class='adminTableHeaderCol_sort'>&#x0110;&#x1ECB;a ch&#x1EC9;</th></tr></thead>";
...
var css ="";
    			...
    			num++;
bien=bien+"<tr class='"+css+"'><td>"+num+"</td><td>"+ pt.dateTime +"</td><td>"+pt.code+"</td><td>"+pt.latlon+"</td><td>"+pt.speed+"</td><td>"+pt.address+"</td></tr>";
}   
document.getElementById('Minhnvbasic-modal-content').innerHTML =bien+"</table></div></div></div></div>";
$('#Minhnvbasic-modal-content').modal();
document.getElementById('simplemodal-container').style.width ='800px';
document.getElementById('simplemodal-container').style.height ='500px';
document.getElementById('simplemodal-container').style.left='200px';
document.getElementById('simplemodal-container').style.top='20px';
$('#chiTietHanhTrinh').tinyscrollbar();
}
...
function jsmFormatCoord(loc, isLat, dec) {
    if ((LATLON_FORMAT == 1) || (LATLON_FORMAT == "DMS")) { // DD^MM'SS"
        ...
        if ((LATLON_FORMAT == 2) || (LATLON_FORMAT == "DM")) {  // DD^MM.mm'
            ...
            var deg = parseInt(loc);
            loc = (loc - deg) * 60.0;
            var min = numFormatFloat(loc, LATLON_FORMAT_MIN_DEC); // minutes
            // decimal
            // places
            if (min <= 9) { min = '0' + min; }
            var quad = isLat ? (isPos ? HEADING[0] : HEADING[4]) : (isPos ? HEADING[2] : HEADING[6]);
...
function jsmCreatePushPin(rcdNdx, dsNdx, ppNdx, evRcd) {
    ...
    var d=evRcd.dateFmt;
    var year=d.substr(0,d.indexOf("/"));
    var month =d.substr(d.indexOf("/")+1,2);
    var day=d.substr(d.lastIndexOf("/")+1,2);
   // var dtime=month;
    var dtime = day+'/'+month+'/'+year + ' ' + evRcd.timeFmt;
    ...
    var statusLastingTime = evRcd.statusLastingTime;
    var driver = evRcd.driver;
    var Cua = "-";
    var DieuHoa = "-";
	var LastFuel =evRcd.LastFuel;
    var account=evRcd.accountID;

    var gpsStr = "";
    if(evRcd.Cua==1) Cua="Đóng";
    else if(evRcd.Cua==0) Cua="Mở";
    if(evRcd.DieuHoa==1 ||evRcd.DieuHoa==0) DieuHoa="Tắt";
    else if(evRcd.DieuHoa==2) DieuHoa="Bật";
    if (sats > 0) {
...
//hienntd add statusLastingTime
    /* balloon text */
    var html = "";
    html += "<table class='infoBoxTable' cellspacing='1' cellpadding='1' border='0'>";
    if(statusLastingTime>0)
    	{ 
    	 var chuoi="";
   	  var gio = parseInt(statusLastingTime / 3600);
   	    var phut =parseInt( (statusLastingTime - gio * 3600) / 60);
   	    var giay = statusLastingTime - gio * 3600 - phut * 60;
   	   // var h="",m="",s="";
   	  ...
   	  //   h = Integer.toString(gio);
   	   ...
   	   // s = Integer.toString(giay);
   	    ...
   	    if(giay>0)
   	    	chuoi+=giay + " giây";
			//+ code + " "
   html += "<tr class='infoBoxRow'><td class='infoBoxCell'>[#" + rcdNdx + "] &nbsp; <b>" + dev + " <br/> " + chuoi+"</b></td></tr>";
    	}
    else
    	{
    	html += "<tr class='infoBoxRow'><td class='infoBoxCell'>[#" + rcdNdx + "] &nbsp; <b>" + dev + " <br/></b></td></tr>";
    	}
    ...
    if (((addr != "") || INCL_BLANK_ADDR)) {
        if (addr == "") { addr = '&nbsp;'; }
        html += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>" + TEXT_INFO_ADDR + ":</b> " + addr + "</td></tr>";
    }
	var str= "";
	if(account=="ctyconghoan")
		str="Tín hiệu cẩu";
	else
		str="Điều Hòa";
	
    html += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Cửa:</b> " + Cua + "</td></tr>";
    html += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>"+str+":</b> " + DieuHoa + "</td></tr>";
    html += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Quãng đường:</b> " + evRcd.odomKM + " KM </td></tr>";
 
	if(LastFuel!="Không có dữ liệu")
    	{
    	var x = LastFuel.split("/");
		if(x[0]!=0)
    html += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Nhiên liệu:</b> " + round_f(x[0],2) + " Lít</td></tr>";
    	}
	...
	if (SHOW_OPT_FIELDS) {
	   ...
	   for (var i = 0; i < evRcd.optDesc.length; i++) {
            	        ...
            	        var d = OptionalEventFieldTitle(i);
                               var r = (d && (d != "")) ? ("<b>" + d + ":</b> " + v) : v;
            	  //   html += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Lái xe:</b> " + r + "</td></tr>";
            	 }
             } 
            	 } 
            	 }  
	if(evRcd.LaiXeHienTai!="laiXeHienTai")
		{
		html += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Lái xe:</b> " + evRcd.LaiXeHienTai + "</td></tr>";
		
		}
	
	if(evRcd.thoiGianLaiXe!="ThoiGianLaiXe")
	{
	html += "<tr class='infoBoxRow'><td class='infoBoxCell'>" + evRcd.thoiGianLaiXe + "</td></tr>";
	
	}
  // html += "<tr class='infoBoxRow'><td class='infoBoxCell'><b>Lái xe:</b> " + driver + "</td></tr>";
    // chua duoc
	//if(LastFuel!="Không có dữ liệu")
	//	{
	//	var x = LastFuel.split("/");
   // html += "<tr class='infoBoxRow'><td colspan='2' align='left'><b>Nhiên liệu:</b>"+round_f(x[0],2)+" Lít</td></tr>";
	//	}
    if(evRcd.lastImageLink!="")
	{html+="<tr><td  ><table><tr><td valign='top' style='font-weight:bold'>Ảnh chụp:</td><td><img src='http://123.30.187.67/ImgGTS/"+evRcd.lastImageLink+"' width='100px' height='100px' onclick='getContent(\""+
	
	evRcd.lastImageLink+"\");' border='1px' alt='không có ảnh' title='Bấm vào đây để phóng to' /></td></tr></table><td></tr>";}
    ...
    if (typeof (REPORT_MAP) != 'undefined') ppi = jsmGetPushPinIconForScoring(evRcd);
	
		//hienntd add
	if(evRcd.code=="Dừng, đang hoạt động")
		{
		//alert("huuhu");
		  ppi = jsmGetPushPinIconForStatusCode(evRcd);
		}
	
    iconUrl.push(ppi.iconURL);
...
function jsmGetPushPinIcon(icon, e) {
    ...
    if (pp.iconEval) {
        try {
            var url = eval(pp.iconEval); // 'e' may be used within this
            // 'eval'
            pp.iconURL = url;
        } catch (err) {
            // Exceptions are possible, since we cannot control what the
            // configuration has specified
            // for the evaluated string. Also note that this Javascript 'eval'
            // trusts the authority
            // of the configuration admin to not place rogue code into the
            // 'iconSelector'
            // alert("Pushpin error: " + err);
            if (!jsvPushpinIcon[0].iconEval) {
                pp.iconURL = jsvPushpinIcon[0].iconURL; // default to black icon
            } else {
                pp.iconURL = ""; // unknown icon (icons will show as broken
                // images)
            }
        }
    }
...
function jsmCreateCircle(center, radiusM) {
    ...
    for (x = 0; x <= 360; x += 5) {         // 5 degrees (saves memory, & it
...
/**
* ** Returns a point that is 'radiusM' from the specified lat/lon in the
* 'heading' direction **
* 
* @param lat
*            The Latitude **
...
function jsmShowDetailPushpin(dsNdx, ppNdx) {
	...
	//hienntd edit
	document.getElementById(dsNdx).style.background="#008EC1";
	//alert(""+countdev);
    for(var i =0;i<countdev;i++)
	{
	  ...
	  if (i!= dsNdx && k!=null)
	  {
	    //alert("hoho"+k+"hehe"+countdev);
	    document.getElementById(i).style.background="#0B1727";
		
	 }
	 }

}
/* NghiepN add this function to push data to div */


/* NghiepN add this function to push data to div END */
/**
...
function jsmShowDetailReport() {
    ...
    for (var i = 0; i < dpt.length; i++) {
        ...
        var x =pt.LastFuel.split("/");
		html+="<td nowrap class='"+dataClass+"' align='left'>" + round_f(x[0],2) + "&nbsp;</td>";
        html += "</tr>\n";
    }
    html += "</tbody>\n";
    html += "</table>\n";
...
*/
/**
* ** Hienntd add Shows the DeviceList table
*/
function jsmLoadDevSearch() {
    	if(document.getElementById('Dev').value !=null);
	  {
	   var searchTextElem =document.getElementById('Dev').value.toLowerCase()
	  }
    if (searchTextElem == "") {
		countdev=0;
        jsmShowDeviceListReport()
        ...
        /* generate HTML table eader */
        var html = "";
        var h = document.getElementById(ID_MAP_CONTROL).clientHeight;
        html += "<div style='width:256px;display: auto; margin: 0 0;'>";
        html += "<table cellspacing='0' cellpadding='0' width='256px'>\n";
        html += "<tbody>\n";
        var lastDevice = "";
        for (var i = 0; i < dpt.length; i++) {
            var pt = jsvDetailAscending ? dpt[i] : dpt[dpt.length - i - 1]; // JSDetailPoint
            ...
            var dataClass = isNew ? CLASS_DETAILS_DATA_COL_NEW : CLASS_DETAILS_DATA_COL;
            lastDevice = pt.device;
            var date = new Date(pt.dateTime);
            // class
            var dev = new RegExp(searchTextElem);
            var kq = dev.test(pt.device.toLowerCase());
            if (kq == true) {
                // alert(pt.device);
              
                html += "<tr><td><div id='"+pt.dsNdx+"' style='border-bottom: solid 1px white;font-size: 0.8em;cursor: pointer;' onclick=\"javascript:" + winVar + ".jsmShowDetailPushpin(" + pt.dsNdx + "," + pt.ppNdx + ");minhnv(" + pt.speed + ");dongHoXang(0);\">";
                html += "<table width='255px'><tbody><tr><td width='50' align='center' rowspan='5'>";
                html += "<img style='padding: 2px;' src='" + iconUrl[i] + "' title='SID: 28183'><br>";
                if (pt.speed != "0.0") {
                    html += "<img height='14' width='14' class='vehListItemImage' src='./images/road.gif'><br>" + pt.speed + "km/h</td>";
                    if (pt.heading > 0 && pt.heading < 90) {
                        html += "<td><img height='12' width='12' title='North' alt='north' src='./images/small_blue_north_east.png'>&nbsp;<em>";
                    }
                    else {
                        if (pt.heading > 90 && pt.heading < 180)
                        { html += "<td><img height='12' width='12' title='North' alt='north' src='./images/small_blue_south_east.png'>&nbsp;<em>"; }
                        else {
                            if (pt.heading > 180 && pt.heading < 270)
                            { html += "<td><img height='12' width='12' title='North' alt='north' src='./images/small_blue_south_west.png'>&nbsp;<em>"; }

                            else {
                                if (pt.heading > 270 && pt.heading < 360)
                                { html += "<td><img height='12' width='12' title='North' alt='north' src='./images/small_blue_north_west.png'>&nbsp;<em>"; }
                                else {
                                    if (pt.heading == 90)
                                    { html += "<td><img height='12' width='12' title='North' alt='north' src='./images/small_blue_east.png'>&nbsp;<em>"; }

                                    else {
                                        if (pt.heading == 180)
                                        { html += "<td><img height='12' width='12' title='North' alt='north' src='./images/small_blue_south.png'>&nbsp;<em>"; }
                                        else {
                                            if (pt.heading == 270)
                                            { html += "<td><img height='12' width='12' title='North' alt='north' src='./images/small_blue_west.png'>&nbsp;<em>"; }
                                            else {
                                                if (pt.heading == 360 || pt.heading == 0)
                                                { html += "<td><img height='12' width='12' title='North' alt='north' src='./images/small_blue_north.png'>&nbsp;<em>"; }
                                            }
                                        }
                                    }
                                }
                    ...
                    html += "<img height='14' width='14' style='padding: 2px;' src='./images/park.png'></td>";
                    html += "<td><img height='12' width='12' title='North' alt='north' src='./images/pause.png'>&nbsp;<em>";
                }
                // html+="<td><img height='12' width='12' title='North'
                // alt='north' src='./images/small_blue_south.png'>&nbsp;<em>";
                html += "<span title='358281007285756' style='font-weight:bold;'>" + pt.device + "&nbsp;</span></em></td></tr>";
                html += "<tr><td>" + pt.dateTime.substring(0, 10) + " " + checkTime(date.getHours()) + ":" + checkTime(date.getMinutes()) + "&nbsp;</td></tr>";
               if(pt.LastFuel!="Không có dữ liệu")
            	   {
            	   var x =pt.LastFuel.split("/");
					html+="<tr><td><b>Nhiên liệu:</b>"+round_f(x[0],2)+"%</td></tr>";
            	   }
				html += "<tr><td title='53959'>" + pt.address + "&nbsp;</td></tr>";
				
			  
				//tanvm: them link den trang bao cao chi tiet cua xe
                html+="<tr><td title='53959'><a href='#'  onclick=\"javascript:openResizableWindow('Track?r_report=EventDetail&page=report.show&page_cmd=rptsel&r_menu=menu.rpt.devDetail&r_format=html&source=PopupFromTicket&date_fr="+pt.dateTime.substring(0,10)+" 00:00&date_to="+pt.dateTime.substring(0,10)+" "+ checkTime(date.getHours())+":"+checkTime(date.getMinutes())+"&device="+pt.deviceId+"','report',900,700)\">Báo cáo chi tiết</a></td></tr>";
   			 	html+="</tbody></table>";
                html += "</div>";
                html += "</td></tr>\n";
            }
        ...
        if (DETAILS_WINDOW) {
            ...
            // inline (under map)
            var tableHTML = "";
            /*tableHTML += "<div style='overflow-y: scroll; height: " + MAP_HEIGHT + "px;border-top:1px solid black;'>" + html + "</div>";
            tableDiv.innerHTML = tableHTML;
            tableDiv.style.display = "block";*/
            
			//tanvm: tao thanh cuon
            tableHTML += "<div id='scrollbar1'>";
    		tableHTML += "<div class=\"scrollbar\"><div class=\"track\"><div class=\"thumb\"><div class=\"end\"></div></div></div></div>";
    		tableHTML += "<div class=\"viewport\" style='height:"+(h-3)+"px'>";
    		tableHTML += "<div class=\"overview\">";
    		tableHTML += html;
    		tableHTML += "</div>";
    		tableHTML += "</div>";
    		tableHTML += "</div>";
    ...
    // content.href='javascript:jsmSearchDevice()';

}
function jsmShowDeviceListReport() {
    ...
    for (var i = 0; i < dpt.length; i++) {
        ...
        html += "<tr><td><div id='"+pt.dsNdx+"'  style='border-bottom: solid 1px white;font-size: 0.8em;cursor: pointer;' onclick=\"javascript:" + winVar + ".jsmShowDetailPushpin(" + pt.dsNdx + "," + pt.ppNdx + ");minhnv(" + pt.speed + ");dongHoXang(" + x[1] + ");\">";
        html += "<table width='255px'><tbody><tr><td width='50' align='center' rowspan='5'>";
        html += "<img style='padding: 2px;' src='" + iconUrl[i] + "' title='SID: 28183'><br>";
        if (pt.speed != "0.0") {
            html += "<img height='14' width='14' class='vehListItemImage' src='./images/road.gif'><br>" + pt.speed + "km/h</td>";
		...
		if(pt.LastFuel!="Không có dữ liệu")
			{
			var x =pt.LastFuel.split("/");
			 html+="<tr><td><b>Nhiên liệu:</b>"+round_f(x[0],2)+" Lít</td></tr>";
			}
        html += "<tr><td title='53959'>" + pt.address + "&nbsp;</td></tr>";
...
/**
* ** Hienntd end add Shows the DeviceList table
*/

/* diepnb add function to standard time */
...
* Developer's Guide: http://code.google.com/apis/chart/ ** Usage Policy :
* http://code.google.com/apis/chart/#usage
*/
function jsmCustomMapMarker(W, H, C) {
    // The following assumptions are made regarding the generated map marker
    // image:
...
/**
* ** Calculate and return a color based on the event speed. ** This algorithm
* performs the following: ** - If the speed is greater than 70 mph, the
* returned color is green. ** - If the speed is between 20 and 70 mph, the
* color will be a mix between yellow and green. ** - If the speed is between 0
* and 20 mph, the color will be a mix between red and yellow. **
* 
* @param e
*            The 'MapEventRecord' object
*/
...
/**
* ** Returns an icon URL based on the event speed. ** Analyzes the event
* 'speed' and creates an icon marker URL on the fly that fades from ** RED
* (stopped) to YELLOW (slow) to GREEN (fast). It uses the Google Charts API to
* create the map ** marker icon [http://code.google.com/apis/chart/]. The first
* 2 arguments are the desired icon ** width and height (required since it is
* dynamically creating this icon). ** Example usage (in the Pushpins tag,
* within a MapProvider): ** <Pushpin key="moving"
* eval="evSpeedMarkerURL(16,24,e)" iconSize="16,24" iconHotspot="8,24"/> **
* 
* @param W
*            The pushpin width **
* @param H
...
* slow, and 'red' if stopped. ** Note: "CrosshairXXXX.gif" images may need to
* be replaced with your *** Custom images, if not present in this version. **
* 
* @param e
*            The 'MapEventRecord' object
*/
...
function dongHoXang(xang) {
	var divXang = document.getElementById("fuel");
	if(divXang!=undefined)
	{
//	if(xang>0)
		divXang.style.display ="block";
//	else 
//		divXang.style.display ="none";
		}
    $("#xang").rotate(
		     1.8 * xang - 90
...
function MinhSetCenter(lat, lon, zoom, DiaDiem) {
    ...
    pp.lon = lon;
    pp.accRadM = 30;
    pp.iconUrl = "images/led/led16_red.png";
    pp.html ="<div style='color:black'>"+ DiaDiem+"</div>";
    addMarkers(pp);

}
*/

function addMarkers(pp) {
    try {
        return jsmap ? jsmap._addPushpin(pp) : false;
    } catch (e) {
        return false; // "JSDrawShape" not defined?
    }
}
...
function removeMarker(lat, lon) {
    ...
    var latn = new jsNewGLatLng(lat, lon);
    var marker = new jsNewImageMarker(latl);



}
var countMinhNV = false;
function full_screen() {

    if (countMinhNV == false) {

        document.getElementById('container').style.display = "none";
        // document.getElementById(ID_MAP_CONTROL).style.display="none";
        // document.getElementById('chonTram').style.display ="none";
        // document.getElementById('minhDevice').style.marginTop="-30px";
        // document.getElementById('minhNVMap').style.marginTop="-35px";
        document.getElementById('page').style.marginTop = "-30px";
        ...
        // trackMapOnLoad();
        countMinhNV = true;
        // document.getElementById('chonTram').style.top ="-360px";
        // document.getElementById('titleChooseTram').style.top ="-380px";
    }
    else {
        document.getElementById('container').style.display = "block";
        document.getElementById(ID_MAP_CONTROL).style.display = "block";
        // document.getElementById('chonTram').style.display ="block";
        document.getElementById('minhDevice').style.marginTop = "0px";
        document.getElementById('minhNVMap').style.marginTop = "0px";
        document.getElementById('page').style.marginTop = "0px";

        jsmLoadDevSearch();
        // document.getElementById('trackMapDataTable').style.height=document.getElementById(ID_MAP_CONTROL_BAR).clientHeight
        // - 33;

        mapControlVisible = true;
        countMinhNV = false;
        // document.getElementById('chonTram').style.top ="-330px";
        // document.getElementById('titleChooseTram').style.top ="-350px";
    }


}

function HideShowTram() {

    if (document.getElementById('chonTram').style.display == "block") {

        document.getElementById('chonTram').style.display = "none";
        document.getElementById('titleChooseTram').style.display = "none";


    }
    else {
        document.getElementById('chonTram').style.display = "block";
        document.getElementById('titleChooseTram').style.display = "block";

    }
}


// Het khu vuc cua MinhNV
function evDeviceNameIconURL(e, fill, border) {
    ...
    var fpt = "10"; // font point size (TODO: check for letters/digits in
    // the text)
...
*            The 'MapEventRecord' object **
* @param x
*            The 'optional' field index with the event object
*/
...
/**
* ** Returns a label icon URL which includes an arrow pointing in the specified
* direction
*/
...
* ** Device ping
*/
function mapDevicePing(pingURL) { // required function
...
*/
function mapProviderParseXML(xmlText) { // required function
    jsMapInit();
...
*/
function mapProviderUpdateMap(mapURL, recenterMode, replay) { // required function
...
*/
function mapProviderToggleDetails() { // required function
    jsvDetailVisible = DETAILS_WINDOW ? true : !jsvDetailVisible;
    jsmShowDetailReport();
};
/**
* **Hienntd add Display "Devicelist" report if it is hidden, else hide
*/
function mapDevicelistDetails() { // required function
    jsvDetailVisible = DETAILS_WINDOW ? true : !jsvDetailVisible;
    jsmLoadDevSearch();
};
...
*/
function mapProviderUnload() { // required function
    jsmUnload();
};
