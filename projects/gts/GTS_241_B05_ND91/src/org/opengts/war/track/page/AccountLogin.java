      ...
      //final boolean legacy = this.getProperties().getBoolean(PROP_legacyLAF,false);
        ...
        HTMLOutput HTML_CONTENT = new HTMLOutput(cssAccountLogin, pageMsg) {
            public void write(PrintWriter out) throws IOException {
                ...
                out.print("<tr><td colspan='2'><img src='images/icon-dn.png' style='position: absolute; top: 115px; left: 670px;'><div class='MinhnvLoginSys'>&#x0110;&#x0103;ng nh&#x1EAD;p h&#x1EC7; th&#x1ED1;ng</div></td></tr>");
             /*  out.print("<div style=\"width:100px; float:left; height:35px; background-image:url(images/icon-dn.png); background-repeat:no-repeat;background-position:70px 0\"></div>"
						+"<div style=\"width:250px; float:right; font-family:arial;font-weight:bold; color:#00b4ff; font-size:14pt; text-align:left\">Login System</div>");
               */ out.println("<tr>");
                ...
                if (!StringTools.isBlank(vsepImg)) {
                  ...
                  //  out.print("<img ");
                   ...
                   // if (!StringTools.isBlank(H)) { out.print(" height='"+H+"'"); }
                    ...
                    out.print("<img style='position: absolute; top: 230px; left: 43px;' src='images/img.png'>");
                    out.print("<span style='color:#00B4FF;font-size:18pt;font-weight:bold;position: absolute;top:160px;left:80px'> Giải đáp và hỗ trợ mọi thắc mắc 24/7 </span><br>");
                    out.print("<span style='color:red;font-size:18pt;font-weight:bold;position: absolute;top:195px;left:120px;text-align:center'>0947.480.305 / 0971.924.333</span><br><br>");
                    out.print("<span style='color: #00B4FF; font-size: 12pt; margin-left: 115px;position: absolute;top:230px;left:55px'> 1. Không đăng nhập được hoặc xe mất tín hiệu. </span><br>");
                    out.print("<span style='color: #00B4FF; font-size: 12pt;position: absolute;top:255px;left:169px'> 2. Hướng dẫn sử dụng hoặc đóng cước dịch vụ. </span><br>");
                   out.print("<span style='color: #00B4FF; font-size: 12pt;position: absolute;top:280px;left:169px'> 3. Và bất kỳ mọi thắc mắc khác. </span>");
                    out.print("</td>");
                }

                String cssLoginText = borderedCss? CSS_LOGIN_TEXT_CELL_CENTER : CSS_LOGIN_TEXT_CELL_LEFT;
                out.println("<td class='"+cssLoginText+"' align='center'>");
                out.print("<div class='MinhnvLoginRequest'>Nh&#x1EAD;p t&#x00E0;i kho&#x1EA3;n v&#x00E0; m&#x1EAD;t kh&#x1EA9;u</div>");
               ...
               // out.println(HR);
                ...
                if (acctLogin) {
                    ...
                    out.print("<tr><td class='accountLoginFieldLabel' align='right'>"+i18n.getString("AccountLogin.account","Account:")+"</td>");
                    out.print( "<td class='accountLoginFieldValue'><input id='"+fldID+"' class='MinhnvTextInput' type='text' "+ro+" name='"+Constants.PARM_ACCOUNT+"' value='"+accountID+"' ></td></tr>");
                    out.print("\n");
                    focusFieldID = fldID;
                }
                // user login field
                if (userLogin && emailLogin) {
                    ...
                    out.print("  <tr><td class='accountLoginFieldLabel' align='right'>"+i18n.getString("AccountLogin.userEmail","User/EMail:")+"</td>");
                    out.print(      "<td class='accountLoginFieldValue'><input id='"+fldID+"' class='MinhnvTextInput' type='text' "+ro+" name='"+Constants.PARM_USER+"' value='"+userID+"' ></td></tr>");
                    out.print("\n");
                    if (StringTools.isBlank(focusFieldID)) { focusFieldID = fldID; }
                } else
                if (userLogin) {
                    String fldID = "userLoginField";
                   out.print("  <tr><td class='accountLoginFieldLabel' align='right'>"+i18n.getString("AccountLogin.user","User:")+"</td>");
                    out.print(      "<td class='accountLoginFieldValue'><input id='"+fldID+"' class='MinhnvTextInput' type='text' "+ro+" name='"+Constants.PARM_USER+"' value='"+userID+"' ></td></tr>");
                    out.print("\n");
                    if (StringTools.isBlank(focusFieldID)) { focusFieldID = fldID; }
                } else
                if (emailLogin) {
                    ...
                    out.print("  <tr><td class='accountLoginFieldLabel' align='right'>"+i18n.getString("AccountLogin.email","EMail:")+"</td>");
                    out.print(      "<td class='accountLoginFieldValue'><input id='"+fldID+"' class='MinhnvTextInput' type='text' "+ro+" name='"+Constants.PARM_USEREMAIL+"' value='"+userID+"' ></td></tr>");
                    out.print("\n");
                    if (StringTools.isBlank(focusFieldID)) { focusFieldID = fldID; }
                }
                // password field
                if (showPasswd) {
                    out.print("  <tr>");
                   out.print(    "<td class='accountLoginFieldLabel' align='right'>"+i18n.getString("AccountLogin.password","Password:")+"</td>");
                    out.print(    "<td class='accountLoginFieldValue'><input class='MinhnvTextInput' type='password' "+ro+" name='"+Constants.PARM_PASSWORD+"' value='' ></td>");
                    out.print(  "</tr>\n");
                }
                // language selection
                if (showLocale) {
                ...
                // Login
                out.print("<br>");
                out.print("<div class='btnSubmit'><table><tr><td><input type='submit' name='submit' value='' class='loginMinhNV'></td></tr>\n");
                // forgot password
                if (showPasswd && (forgotURL != null)) {
                   if (legacy) { out.println("<br>"); }
                   out.println("<tr style='height:30px'><td><font color ='blue'><i><a href='"+forgotURL+"'>"+i18n.getString("AccountLogin.forgotPassword","Forgot your password?")+"</a></i></font></td></tr></table></div>");
                }
                // end forn
                out.println("</form>");
               ...
               // out.println("<br/>");
                ...
                out.println("</tr>");
                out.println("</table>");
               // out.print("<div class='MinhnvLogo'><img src='images/Satellite.png' width='150px'/></div>");
